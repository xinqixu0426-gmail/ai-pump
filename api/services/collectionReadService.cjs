...
const {projectCollectionDetail}=require('./collectionDetailProjection.cjs');
...
const SQL={
    ...
    parts: {
        ...
        detail: "SELECT id, model AS name, category, supplier, remark FROM parts WHERE deleted_at IS NULL AND ((:targetId IS NOT NULL AND id=:targetId) OR (:identity IS NOT NULL AND model=:identity)) ORDER BY id DESC LIMIT 2"
    },
    recipes: {
    ...
    function read(input){
        ...
        const result=db.transaction(()=>{
            ...
            const items=fetched.slice(0,q.operation==='detail'?1:q.pageSize).map(row=>{
                ...
                const {id,...listDisplay}=row;
                const display=q.operation==='detail'?projectCollectionDetail(q.resourceType,row):listDisplay;
