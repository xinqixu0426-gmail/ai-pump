...
function normalizePartNaming(naming, category, model) {
    ...
    if (generated.category !== String(category).trim()) fail('PART_NAMING_CATEGORY_MISMATCH', '命名规则与零件分类不一致');
...
function assertPartNamingUpdate(current, updates, naming) {
    ...
    const normalized = normalizePartNaming(
        naming === undefined ? { ruleId: stored.ruleId, spec: stored.spec } : naming,
        updates.category ?? current.category,
        updates.model ?? current.model
    );
    if (JSON.stringify(normalized.stored) !== JSON.stringify(stored)) {
        fail('PART_NAMING_CHANGE_REQUIRES_REVIEW', '不能通过普通编辑改变命名规格；请先核实实物规格和引用', 409);
    }
}
