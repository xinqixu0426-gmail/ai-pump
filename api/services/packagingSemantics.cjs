...
function inferPackagingSemantics(part = {}) {
    ...
    if (haystack.includes('珍珠棉')) {
        return { packagingMaterial: '珍珠棉', packingRole: 'pearlCotton' };
    }
    if (haystack.includes('泡沫')) {
        return { packagingMaterial: '泡沫', packingRole: 'foam' };
    }
    if (haystack.includes('说明书')) {
        return { packagingMaterial: '说明书', packingRole: 'fixed' };
    }
    if (haystack.includes('贴纸') || haystack.includes('商标')) {
        return { packagingMaterial: '其他包材', packingRole: 'fixed' };
    }
    if (haystack.includes('木箱')) {
        return { packagingMaterial: '木箱', packingRole: 'container' };
    }
    if (haystack.includes('彩印') || haystack.includes('彩箱')) {
        return { packagingMaterial: '彩印箱', packingRole: 'container' };
    }
    if (haystack.includes('牛皮')) {
        return { packagingMaterial: '牛皮纸箱', packingRole: 'container' };
    }
