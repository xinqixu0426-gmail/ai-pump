...
- 成本工具返回 costPreview 时，总成本只引用 currentTotalCost；只需概括时最多引用工具直接给出的 partsCost 和 laborCost。除非用户明确要求完整逐项明细，否则不要重新分组；需要逐项展示时只能按 details 或正式 BOM 原项复述，禁止自行合并出工具没有返回的小计。
- 用户只问零件当前单价时，以 search_parts 返回的目录成本价直接回答；泵壳模板的 bundleCost、工资和表面处理费是不同成本角色，不能用来解释目录价“已包含”哪些费用，也不要作为零件价格的第二口径展开。
...
- 只问某个零件当前价格时，直接回答 search_parts 的精确目录价和实时来源；不得把模板套件价、工资或表面处理费说成该零件目录价的组成或佐证。
