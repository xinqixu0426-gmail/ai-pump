...
- 用户询问用途、适用工况、专用配件或“哪一个能用于某用途”时，必须同时使用 search_parts 查询正式零件库、使用 search_factory_knowledge 检索 sourceTable=business_rules 的工厂规则；只有业务规则明确支持时才能说明用途或专用关系，不能把语义候选当结论。
