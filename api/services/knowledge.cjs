...
function partEntry(part) {
    return createEntry({
        ...
        summary: `${part.category || '其他'}${part.subcategory ? ` / ${part.subcategory}` : ''}，目录成本价 ${Number(part.price || 0)} 元，库存 ${Number(part.stock || 0)}`,
        content: [
            `型号：${part.model || ''}`,
            `分类：${part.category || '其他'}${part.subcategory ? ` / ${part.subcategory}` : ''}`,
            `供应商：${part.supplier || '-'}`,
            `目录成本价：${Number(part.price || 0)} 元`,
            `库存：${Number(part.stock || 0)}`,
            (part.remark ?? part.notes) ? `备注：${part.remark ?? part.notes}` : '',
        ],
...
function recipeEntry(recipe, technicalFiles = []) {
    ...
    return createEntry({
        ...
        title: `成品：${recipe.name || recipe.id}`,
        summary: `${recipe.spec || '未填写配置摘要'}，保存成本 ${Number(recipe.savedTotalCost || 0)} 元，线圈 ${recipe.coilSpec || '-'}-${recipe.coilSheets || '-'}`,
        content: [
            `成品型号：${recipe.name || ''}`,
            `配置摘要：${recipe.spec || ''}`,
            ...
            `BOM 快照：${parts.map(item => `${item.model || item.name || '项目'} x ${item.qty || 1}`).join('；')}`,
