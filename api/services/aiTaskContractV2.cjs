...
const TASK_TRANSITIONS_V2 = Object.freeze({
    ...
    WAITING_INPUT: ['RESOLVING', 'CANCELLED'],
    // NATIVE-W1 §18/§21：未批准的提案允许**重新预览**（自转移 = 用新确认卡替换旧卡），
    // 以及在被批准后转入执行（RUNNING）。
    WAITING_APPROVAL: ['WAITING_APPROVAL', 'RESOLVING', 'RUNNING', 'CANCELLED'],
    VERIFYING: ['SUCCEEDED', 'PARTIAL', 'UNSUPPORTED', 'WAITING_INPUT', 'WAITING_APPROVAL', 'RECONCILING', 'FAILED', 'CANCELLED'],
    SUSPENDED: ['RESOLVING', 'CANCELLED'],
    // NATIVE-W1 §15：RECONCILING 必须有**终局出口**。有界对账用尽后必须以安全失败终止，
    // 不允许任务永久停留在 RECONCILING（PARTIAL 用于取消竞态下的合法部分完成）。
    RECONCILING: ['RECONCILING', 'VERIFYING', 'WAITING_APPROVAL', 'SUSPENDED', 'FAILED', 'PARTIAL'],
