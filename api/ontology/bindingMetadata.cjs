...
const expressions = {
    ...
    'recipe.uses_coil': ['(?<root>.+?)的?(?:使用|用|用了|用的是|配的)(?:哪个|什么)?(?:线圈|绕组)', '(?<root>.+?)的?(?:线圈|绕组)(?:是哪个|是什么)'],
    ...
    'recipe.contains_part': ['(?<root>.+?)的?(?:配件明细|零件明细)(?:有哪些)', '(?<root>.+?)(?:用了哪些|使用哪些|需要哪些|配了哪些)(?:零件|配件)'],
    'part.contained_in_recipe': ['(?<root>.+?)(?:被哪些|给哪些)(?:配方|产品)(?:使用|用|用了)', '(?<root>.+?)(?:用在哪些|装在哪些)(?:配方|产品)(?:里)?'],
};
