...
function createBusinessUnderstandingFixtureV2(options = {}) {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'business-understanding-v2-'));
    ...
    const coil = (ref, values) => {
        ...
        const storedCost = calculateStoredCoilCost({
            pricingMode,
        ...
        ids[ref] = insertId(db, `INSERT INTO coils(spec,sheets,material,slot_type,scheme_code,scheme_name,
            ...
            values.slotType, values.code, values.name, 'official', pricingMode, values.kitPrice ?? 0,
            values.unitPrice ?? 0, storedCost, values.stock, values.copperBase ?? 88,
    ...
    coil('officialCoil.12-140-calculated', { spec: '12', sheets: 140, material: '钢带', slotType: '小眼',
        code: 'C-12-140-CALC', name: '12-140钢带小眼计算价', pricingMode: 'calculated', unitPrice: 0.2,
        copperBase: 88, wireWeight: 0.5, coilFee: 2, rotorFee: 1, stock: 9 });
    coil('officialCoil.12-160-kit', { spec: '12', sheets: 160, material: '钢带', slotType: '小眼',
        code: 'C-12-160-KIT', name: '12-160钢带小眼套件价', pricingMode: 'kit', kitPrice: 76, copperBase: 88,
        wireWeight: 0.6, stock: 6 });
    coil('officialCoil.12-200-steel-small', { spec: '12', sheets: 200, material: '钢带', slotType: '小眼', code: 'C-12-200-A', name: '12-200钢带小眼', kitPrice: 88, stock: 7 });
    coil('officialCoil.12-200-cold-standard', { spec: '12', sheets: 200, material: '冷轧', slotType: '国标眼', code: 'C-12-200-B', name: '12-200冷轧国标眼', kitPrice: 96, stock: 0 });
    coil('officialCoil.12-220-steel-small', { spec: '12', sheets: 220, material: '钢带', slotType: '小眼', code: 'C-12-220-A', name: '12-220钢带小眼', kitPrice: 102, stock: 4 });
    coil('officialCoil.12-220-cold-standard', { spec: '12', sheets: 220, material: '冷轧', slotType: '国标眼', code: 'C-12-220-B', name: '12-220冷轧国标眼', kitPrice: 111, stock: 2 });
...
function runFixtureSelfChecksV2(fixture) {
    const { db, ids } = fixture;
    const checks = [];
    const add = (key, passed, actual) => checks.push({ key, passed: Boolean(passed), actual });
    const variants = (spec, sheets) => db.prepare(`SELECT id,material,slot_type,stock,pricing_mode FROM coils
    ...
    add('BU-05-calculated-pricing', variants('12', 140).length === 1
        && variants('12', 140)[0].pricing_mode === 'calculated'
        && variants('12', 140)[0].id === ids['officialCoil.12-140-calculated'], variants('12', 140));
    ...
    add('BU-11-kit-pricing', variants('12', 160).length === 1
        && variants('12', 160)[0].pricing_mode === 'kit'
        && variants('12', 160)[0].id === ids['officialCoil.12-160-kit'], variants('12', 160));
    return { passed: checks.every(check => check.passed), checks };
}

module.exports = { createBusinessUnderstandingFixtureV2, runFixtureSelfChecksV2 };
