...
test('Next UI 契约：线圈页移除材质默认单价并保留定子组合批量改单价', () => {
    ...
    const coilCost = readUtf8('api/services/coilCost.cjs');
    const copperPriceUpdate = readUtf8('api/services/copperPriceUpdate.cjs');
    ...
    assert.match(coilsLib, /pricingMode: 'calculated' \| 'kit'/);
    assert.match(coilsLib, /kitPrice: number/);
    assert.match(coilsView, /<Field label="计价方式"/);
    assert.match(coilsView, /<option value="kit">供应商套件价<\/option>/);
    assert.match(coilsView, /form\.pricingMode === 'kit'/);
    assert.match(coilsView, /calculatedRows = group\.rows\.filter/);
    assert.match(coilCommands, /normalizedCost\.pricingMode === 'kit'/);
    assert.match(coilCommands, /pricingMode \|\| 'calculated'\) === 'calculated'/);
    assert.match(coilCost, /供应商套件价不参与插值或外推/);
    assert.match(copperPriceUpdate, /pricing_mode \|\| coil\.pricingMode \|\| 'calculated'/);
...
test('Next UI 契约：配方详情独立展示成本、BOM、技术参数和库存', () => {
    ...
    assert.ok(recipeDetailPanel.indexOf('>BOM 快照<') < recipeDetailPanel.indexOf('>关键参数<'));
    assert.doesNotMatch(recipeDetailPanel, /max-h-80|sticky top-0|sticky bottom-0/);
    assert.match(recipeDetailPanel, /bomGroups\.map/);
    assert.match(recipeDetailPanel, /group\.rows\.map/);
    assert.match(recipeDetailPanel, /'pl-8'/);
    assert.match(recipeDetailPanel, /recipeFeeRows/);
    assert.match(recipeDetailPanel, /window\.print\(\)/);
    assert.match(recipeDetailPanel, /<RecipePrintSheet/);
    const recipePrintSheet = readUtf8('apps/web-next/components/recipe/RecipePrintSheet.tsx');
    assert.match(recipePrintSheet, /createPortal/);
    assert.match(recipePrintSheet, /document\.body/);
    assert.match(recipePrintSheet, /group\.rows\.map/);
    assert.match(recipePrintSheet, /recipePartModel\(row.part, recipe\)/);
    assert.match(recipePrintSheet, /fee\.process/);
    assert.match(recipePrintSheet, /document\.title = \(recipe.name/);
    assert.match(recipePrintSheet, /document\.title = previousTitle/);
    assert.match(recipePrintSheet, /<colgroup>/);
    assert.match(recipePrintSheet, /print-overview/);
    assert.doesNotMatch(recipePrintSheet, /inventoryStatus|max-h-|overflow-auto/);
    const printCss = readUtf8('apps/web-next/app/globals.css');
    assert.match(printCss, /@media print/);
    assert.match(printCss, /table-header-group/);
    assert.match(printCss, /body:has\(> \.recipe-print-sheet\) > :not\(\.recipe-print-sheet\)/);
    assert.match(recipeDetailPanel, /<details key=\{recipe.id\}/);
    assert.match(recipeDetailPanel, /<summary[^\n]*配件与线圈库存/);
    assert.match(recipeDetailPanel, /配件与线圈库存/);
