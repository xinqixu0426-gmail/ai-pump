...
test('V5 production import remains limited to approved shadow, interpreter, and governed entity-read boundaries', () => {
    ...
    assert.deepEqual(forbiddenV5Imports, ['candidateChoice.cjs', 'candidateRead.cjs', 'candidateSet.cjs', 'collectionDetailTarget.cjs', 'collectionReadRuntime.cjs', 'investigationRuntime.cjs', 'readExecutionShadow.cjs', 'taskInterpreter.cjs', 'twoStageModel.cjs', 'typeIndependentEntityResolver.cjs']);
    const choiceRuntime = fs.readFileSync(path.join(v5Root, 'candidateChoice.cjs'), 'utf8');
    assert.doesNotMatch(choiceRuntime, /(?:db\.cjs|\bfetch\s*\(|allowWrite:\s*true|putJson|patchJson|deleteJson)/);
    assert.match(choiceRuntime, /verifyCollectionExecution/);
    ...
    assert.deepEqual(observabilityImporters, ['candidateChoice.cjs', 'candidateRead.cjs', 'candidateSetTwoStageInterpreter.cjs', 'collectionReadRuntime.cjs', 'investigationRuntime.cjs', 'readAnswerComposer.cjs', 'readAuthorityMux.cjs', 'readCanary.cjs', 'readExecutionShadow.cjs', 'shadowMirror.cjs']);
