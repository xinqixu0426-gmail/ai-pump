...
function runtime(i) {
    ...
    return {sourceRequest,interpreterModelRequest:async()=>{
        ...
        return {content:JSON.stringify(calls===1?{version:1,spanRef:span.spanRef,needsClarification:false}:{version:1,localTaskClassRef:'tc_002'})};
...
async function main(){
    ...
    assert.equal(mirror.snapshot().v5ModelCalls,20);assert.equal(mirror.snapshot().v5BusinessApiCalls,10);
