...
const output = path.join(root,'docs/ai-governance/data/v5-e4r-nested-span-refinement-evaluation.json');
const hash = value => crypto.createHash('sha256').update(value).digest('hex');
const read = file => JSON.parse(fs.readFileSync(path.join(root,file),'utf8'));
const moduleFiles = ['nestedSpanRefinement','candidateUnion','twoStageModel','sourceSpanSelector','sourceSpanSelectionContract','candidateSet','localTaskClassCatalog','localIntentSelector','localIntentContract','entityFinalization','candidateSetTwoStageInterpreter', 'independentShadow','shadowMirror','typeIndependentEntityResolver','sourceSpanCatalog','sourceAnchoredEntity','taskClassCatalog','taskClassSemantics','capabilityRegistry','capabilityRouter','toolExposure','taskInterpreterInput','taskInterpretationContract','contracts','taskState','controlledRuntime','policy','evidenceLedger'];
function freezeHashes() {
    ...
    files.push('scripts/run-ai-v5e4r-top2-preflight.cjs','scripts/run-ai-v5e4r-top2-infrastructure.cjs');
    files.push('scripts/run-ai-v5e4r-nested-refinement-evaluation.cjs','tests/aiV5NestedSpanRefinement.test.cjs','docs/ai-governance/v5-bounded-nested-span-refinement-v1.md','scripts/run-ai-v5e4r-top2-evaluation.cjs','api/services/entityLookupService.cjs','api/routes/entityLookup.cjs','api/routes/ai/internalApiClient.cjs','api/services/observability.cjs','scripts/run-ai-v5e4r-two-stage-evaluation.cjs','scripts/lib/v5TwoStageEvaluationControl.cjs','scripts/run-ai-v5e4r-two-stage-json-preflight.cjs');
...
function summarizeLegacy(paths) {
...
function summarize(paths) {
    const base=summarizeLegacy(paths);
    const count=fn=>paths.filter(fn).length;
    const sum=key=>paths.reduce((n,p)=>n+(p[key]||0),0);
    return {...base,top1:ratio(count(p=>p.top1Match),paths.length),recallAt2:ratio(count(p=>p.expectedSpanRecallAt2),paths.length),
        resolverCalls:sum('resolverCalls'),deduplications:sum('candidateUnionDeduplications'),
        maxResolverCalls:Math.max(0,...paths.map(p=>p.resolverCalls||0)),
        bothNotFound:count(p=>p.lookupStatuses?.length===2&&p.lookupStatuses.every(s=>s==='NOT_FOUND')),
        oneNotFoundOneHit:count(p=>p.lookupStatuses?.length===2&&p.lookupStatuses.filter(s=>s==='NOT_FOUND').length===1&&p.complete),
        bothHit:count(p=>p.lookupStatuses?.length===2&&p.lookupStatuses.every(s=>['RESOLVED','AMBIGUOUS'].includes(s))),
        unionMedian:percentile(paths.map(p=>p.candidateCount),0.5),unionMax:Math.max(0,...paths.map(p=>p.candidateCount||0))};
}
async function main() {
    ...
    const previous=read('docs/ai-governance/data/v5-e4r-top2-source-span-evaluation.json');
    const allowed=new Set(['api/services/ai-v5/candidateUnion.cjs','api/services/ai-v5/candidateSetTwoStageInterpreter.cjs']);
    const current=freezeHashes();
    for(const [key,value] of Object.entries(previous.preEvalHashes)) {
        if(!allowed.has(key)&&current[key]!==value) throw new Error('FROZEN_COMPONENT_CHANGED');
    }
    ...
    const dataset={version:1,architectureVersion:3,priorInfrastructureAbortedRuns:0,formalRealEvaluationRuns:1,v4TrajectorySource:'FROZEN_P06_ARTIFACT_NO_V4_MODEL_RERUN',preEvalHashes:pre,paths:[],metrics:null};
    ...
    try {
        for(const oracle of expected) {
            ...
            const record=await withAgentSpan({requestId:taskId,route:'v5-shadow-v3'},async()=>{
                ...
                const selectedSpans=(result.sourceSpanRefs||[]).map(ref=>getSourceSpan(createV5SourceSpanCatalog(fixture.source),ref));
                const selectedSpan=selectedSpans[0];
                const meta=result.architectureMetadata;
                const refinedSpans=(meta.nestedSpanRefs||[]).map(ref=>getSourceSpan(createV5SourceSpanCatalog(fixture.source),ref));
                ...
                return {case_id:oracle.case_id,source_group_id:oracle.source_group,inputFingerprint:envelope.inputFingerprint,
                    ...meta,expectedSpanRecovered:[...selectedSpans,...refinedSpans].some(s=>s?.text===fixture.mention),
                    nestedExpectedSpanSelected:refinedSpans.some(s=>s?.text===fixture.mention),
                    originalSuccessfulPath:previous.paths.find(p=>p.case_id===oracle.case_id).finalEntityCorrect,
                    spanMatch:selectedSpans.some(s=>s?.text===fixture.mention),
                    top1Match:selectedSpan?.text===fixture.mention,expectedSpanRecallAt2:selectedSpans.some(s=>s?.text===fixture.mention),
                    expectedSpanRank:selectedSpans.findIndex(s=>s?.text===fixture.mention)+1,
                    candidateUnionComplete:meta.complete,candidateUnionCount:meta.candidateCount,
                    finalEntityTypeMatch:result.resolvedIdentity?.entityType===oracle.expected_entity_type,
                    ...
                    capabilityMatch:comparison.capabilityMatch,expectedToolExposed:comparison.expectedToolExposed,wrongToolExcluded:result.status==='VALID'&&result.taskClassRef===oracle.expected_class&&comparison.capabilityMatch&&comparison.wrongToolExcluded,r02:item.failure_class==='R02',
            ...
            for(const fixture of fixtures.values()) {
                if(JSON.stringify(record).includes(fixture.source)||JSON.stringify(record).includes(fixture.mention)) throw new Error('PRIVACY_INVARIANT_FAILED');
            }
