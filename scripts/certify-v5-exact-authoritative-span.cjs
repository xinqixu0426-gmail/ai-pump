...
const protectedFiles = ['api/services/ai-v5/exactAuthoritativeSpan.cjs','api/services/entitySpanCandidates.cjs','api/routes/entitySpanCandidates.cjs','api/routes/ai/internalApiClient.cjs',
...
async function main() {
    ...
    const data={stage:'P16-I-R9',formalRuns:1,retry:0,paths:[],spanPreflight:[],writes:0,ownerDefaultEnabled:false,preHashes:hashes()};
    ...
    const runtime=await require('./start-v5-candidate.cjs').startCandidate({readOptions:async()=>({
        interpret:async(envelope,options)=>{
            ...
            checks={interpreterStatus:r.status,interpreterReason:r.reasonCode,stage1Status:m.stage1Status,stage2Status:m.stage2Status,
                ...
                stage1Calls:m.stage1Calls,stage2Calls:m.stage2Calls,fastPath:m.exactAuthoritativeFastPath,authoritativeSpanCount:m.authoritativeSpanCount,
                fastPathDurationMs:m.exactSelectionDurationMs,
    ...
    try {
        ...
        data.pass=!data.error&&data.paths.length===3&&data.paths.every(p=>p.mappingMatch&&p.identityMatch&&p.entityTypeMatch&&p.exactIdentityMatch
            &&p.fastPath&&p.stage1Calls===0&&p.authoritativeSpanCount===1&&p.stage2Status==='VALID'
