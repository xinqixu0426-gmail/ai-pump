...
const output = path.join(root, 'docs/ai-governance/data/v5-f1-read-execution-shadow-evaluation.json');
...
const files = ['api/services/ai-v5/readExecutionRegistry.cjs', 'api/services/ai-v5/readArgumentBinder.cjs',
...
async function openFixture() {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pump-v5-f1-'));
...
async function main() {
    ...
    const prior = read('docs/ai-governance/data/v5-e4r-nested-span-refinement-evaluation.json');
    const hashes = freezeHashes();
    for (const [key, value] of Object.entries(prior.postEvalHashes)) {
        if (!['api/services/ai-v5/independentShadow.cjs', 'api/services/ai-v5/shadowMirror.cjs'].includes(key)) assert.equal(hashes[key], value, key);
    }
    ...
    try {
        if (preflight) {
            ...
            observation.initializeObservability({ env: { ...process.env, AI_OBSERVABILITY_ENABLED: 'true', AI_TRACE_CONTENT: 'metadata', AI_OBSERVABILITY_PROJECT: 'pump-ai-v5-f1-read-execution' } });
