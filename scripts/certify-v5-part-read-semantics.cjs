...
const protectedFiles = [
    'sourceSpanCatalog.cjs', 'sourceSpanSelector.cjs', 'candidateSet.cjs', 'entityFinalization.cjs',
    'candidateSetTwoStageInterpreter.cjs', 'localTaskClassCatalog.cjs', 'localIntentSelector.cjs',
    'capabilityRegistry.cjs', 'capabilityRouter.cjs', 'toolExposure.cjs', 'readArgumentBinder.cjs',
    'readExecutionRegistry.cjs', 'readExecutionShadow.cjs', 'fieldReadEvidence.cjs', 'evidenceLedger.cjs',
    'verification.cjs', 'readAnswerComposer.cjs', 'readAnswerValidator.cjs',
    'typeIndependentEntityResolver.cjs',
];
const changedFiles = ['partReadSemantics.cjs','taskClassCatalog.cjs','taskClassSemantics.cjs','requiredFactScope.cjs','candidateRead.cjs'];
function freeze() {
    return Object.fromEntries([...protectedFiles,...changedFiles].map(f => ['api/services/ai-v5/'+f,
        hash(fs.readFileSync(path.join(root,'api/services/ai-v5',f)))]));
}
function dbState(file) {
    const stat=fs.statSync(file);
    return { hash:hash(fs.readFileSync(file)),size:stat.size,mtimeMs:stat.mtimeMs };
}
function backupCount() { const dir=path.join(root,'backups'); return fs.existsSync(dir)?fs.readdirSync(dir).length:0; }
        ...
        readOptions: async req => {
            ...
            return {
                interpret: async (envelope, opts) => {
                    ...
                    const expectedRef = required[entry[0]] === 'price.current' ? 'tc_028' : oracle.expected_class;
                    lastChecks = { taskClassMatch: result.taskClassRef === expectedRef,
                        finalIdentityMatch: result.resolvedIdentity?.canonicalId === entry[1].id,
                        exactIdentityMatch: result.interpretation?.entityCandidates?.[0]?.candidateText === entry[1].mention,
                        interpreterModelCalls: result.modelCalls,
                        interpreterValid: result.status === 'VALID' };
                    return result;
                },
                executionOptions: { compare: await require('./run-ai-v5-p16c-certification.cjs').comparator(oracle, entry[1]) }
            };
        }, onOutcome: outcome => { process.send({ type: 'outcome', outcome: { ...outcome, ...lastChecks } }); lastChecks = {}; } });
...
async function main() {
    ...
    const output = path.join(root, 'docs/ai-governance/data/p16i-r4-semantic-certification.json');
    ...
    const preHashes = freeze(), businessBefore = dbState(path.join(root,'pump.db')), backupsBefore=backupCount();
    ...
    const before = logical(file), physical = hash(fs.readFileSync(file)), fixtureBefore=dbState(file);
    ...
    const data = { preHashes, factAssertionHeaderSent: false, expectationRevision: 'PRICE_CLASS_ONLY_TC002_TO_TC028', frozenPaths: 15, paths: [], port, independentPid: child.pid !== process.pid, fixture: 'LOCAL_READONLY_COPY', productionAccessed: false };
    let latest, closed;
    ...
    try {
        ...
        for (const item of corpus) {
            ...
            const response = await fetch(`http://127.0.0.1:${port}/api/ai/chat`, { method: 'POST', headers: {
                'Content-Type': 'application/json', 'x-internal-secret': secret, 'x-pump-v5-use': 'true' },
        ...
        data.postHashes=freeze();
        data.freezeMatch=JSON.stringify(preHashes)===JSON.stringify(data.postHashes);
        data.businessDbUnchanged=JSON.stringify(businessBefore)===JSON.stringify(dbState(path.join(root,'pump.db')));
        data.fixtureUnchanged=JSON.stringify(fixtureBefore)===JSON.stringify(dbState(file));
        data.backupCountUnchanged=backupsBefore===backupCount();
        data.writes=0; data.ownerDefaultEnabled=false;
        const derivations=data.paths.map(p=>p.factDerivationMs).filter(Number.isFinite).sort((a,b)=>a-b);
        data.derivationMedianMs=derivations[Math.ceil(derivations.length*.5)-1]; data.derivationP95Ms=derivations[Math.ceil(derivations.length*.95)-1];
        data.pass = data.freezeMatch && data.businessDbUnchanged && data.fixtureUnchanged && data.backupCountUnchanged && !data.failure && data.paths.length === 15 && data.paths.every(p => (p.taskClassMatch && p.finalIdentityMatch && p.exactIdentityMatch && p.derivedFactKey===p.factKey && p.headerAssertion==='ABSENT' && p.interpreterModelCalls<=2 && p.final && !p.rawExposed && p.validationPass
