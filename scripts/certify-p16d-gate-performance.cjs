...
async function client(port) {
    return new Promise((resolve,reject) => {
        ...
        } catch { reject(Error('P16D_TRANSPORT_FAILED')); } });
    });
}
async function main() {
    process.env.NODE_ENV = 'test'; process.env.NODE_TEST_CONTEXT = '1'; process.env.AI_OBSERVABILITY_ENABLED = 'false';
    const output = path.join(root, 'docs/ai-governance/data/p16d-gate-performance.json');
    ...
    baseline._compile(execFileSync('git', ['show', '362d107fc8eb7a593c4251db7e420d96115aae1d:api/routes/ai/chat.cjs'], { cwd: root }).toString(), filename);
    ...
    const data = { version: 1, baselineCommit: '362d107fc8eb7a593c4251db7e420d96115aae1d',
...
if(require.main===module) main().catch(()=>{ console.error('P16D_PERFORMANCE_STOPPED'); process.exitCode=1; });
