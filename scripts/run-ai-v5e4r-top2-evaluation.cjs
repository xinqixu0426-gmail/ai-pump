...
async function main() {
    ...
    const preflight=read('docs/ai-governance/data/v5-e4r-top2-preflight.json');
    if(preflight.stage1?.status!=='VALID') throw new Error('PREFLIGHT_CANARY_FAILED');
        ...
        for(const oracle of expected) {
            ...
            const record=await withAgentSpan({requestId:taskId,route:'v5-shadow-v3'},async()=>{
                ...
                return {case_id:oracle.case_id,source_group_id:oracle.source_group,inputFingerprint:envelope.inputFingerprint,
                    ...meta,spanMatch:selectedSpans.some(s=>s?.text===fixture.mention),
