...
### 2026-09-07 V1 修复发布状态：未通过，生产已恢复稳定版

- 待发布代码：`5f1fc21`（已推送）；生产运行：`12fee179b6074215cf359bcc1a789ce1a345b9ba`、迁移 79、原 Web 构建。V1 冻结标签未移动。
- 最终本地及 Mac Mini 代码门禁：测试 **1861/1861**、API 契约 **26/26**、深度 API **437/437**，lint、生产构建、依赖审计及生产环境检查通过。代码门禁通过不代表真实模型验收通过。
- 最新生产真实 AI **6/9**。未解决：查询已有型号仍提前反问、不继续查询正式零件单价；提出超出剩余工具预算的一组调用时未完成最终回答；“没有记录到任何历史报价”仍被零报价确定性规则误判。不得把这些问题归入历史 M-24，也不恢复旧框架或放宽写权限。
- 本次已实现：正式金额来源和总价汇总、模板价格含义、目标缺失与空查询反馈、内部协议拦截及最终回答协议转换、知识文本去重。评测补齐了等价来源、绕组身份位置、排除候选、部分零结果表达判定；真实模型完成查询的稳定性仍不足。
- 最终回滚备份：`pump-release-2026-09-07T10-06-28-363Z.db`。回滚后公网 ready=true、运行 commit=`12fee179b607`、migration=79、启动备份通过，登录页和 AI 页面 HTTP 200。生产 `.env` 与最初发布前逐字节相同，未启用业务写操作；最终失败期间逐表对比仅有迁移、评测、审计/operation、同步和 FTS 内部表变化，原有业务表未变。隔离 3402 服务已停止。

#### 当前继续修复（待生产验收）

- 已在公共循环修复提前反问的单次完成性复核、整组提议超出次数时转入最终汇总；实际候选歧义保留显式选择，未增加查询或写权限。零报价判定补齐“没有记录到”“不存在该客户的报价记录”，并保留假设、疑问和双重否定拒绝。
- 隔离运行 56 为 8/9、57 为 7/9，原三项失败均通过；新暴露的未发送草稿引发补充回答、明确拒绝替代来源被误判，已修复并增加回归。配置成本专项运行 58 为 1/1。真实“查一下最近 5 个订单”正常返回现有 1 条，无超容。
- 上轮本地全量 1862/1862、契约 26/26、深度 API 437/437、lint 和 build 通过；随后 `6d555d0` 生产 8/9，原三项均通过，配置成本因模板多关键词零命中与绕路查询耗尽次数未通过。已按发布前备份 `pump-release-2026-09-07T10-30-46-458Z.db` 回滚稳定版。
- 继续修复正式模板目录的多关键词匹配，并在模板模型视图提供既有 BOM 试算入口，不增加写权限或查询次数。隔离真实运行 59 为 9/9；本地测试 1863/1863、契约 26/26、lint 和 build 通过。深度 API 首次外部铜价 fetch failed，已保留日志并确认来源恢复连接，复核 437/437 通过；待生产验收。
...
- 本机全部日志与只读复核结果：`C:\Users\Dan\Documents\pump-v1-release-archive\2026-09-07`。最终发布日志为 `deployment-zero-quotation.log`，数据库对比为 `5f1fc21-database-diff.json`。后续应修复共享的查询完成机制及零结果判定，使用保存的失败回执覆盖回归；不能继续堆提示、重复运行直到偶然全绿。新版尚未进入最终生产 MCP 全领域验收。
