...
### 2026-09-07 正式发布尝试：未通过，已回滚

- 待发布源码 `d81ff7e3e85f713f5f6ca265fc93b5d772588d41`（V1 加文档整理）已推送。生产机器完整代码门禁通过，主服务曾切换并完成迁移 80；真实 AI 发布检查运行 47 仅 **4/9**，因此发布失败，未进入后续生产 MCP 全领域验收。
- 失败包括零件目录价使用模板来源、缺失目标混入其他资料、用途不确定性表达，以及成本兜底摘要漏掉正式总价和配置条件。不得把该结果改写为成功或与历史 M-24 合并归因。
- 已恢复旧源码 `12fee179b6074215cf359bcc1a789ce1a345b9ba`、原 Web 构建和迁移 79 数据库。恢复前已完整保存失败数据库与日志；对比所有原有表，变化仅为迁移、评测、审计/operation、同步记录及 FTS 内部表，业务表未变。生产 `.env` 与发布前逐字节一致，未新增写权限。
- 回滚后 API ready=true、运行 commit=`12fee179b607`、migration=79、启动备份通过；公网登录页和 AI 页面均 HTTP 200。临时验收服务已停止。
- 成本摘要修复及业务说明调整已在本地验证，全量测试 **1849/1849**。随后生产数据副本上的隔离检查运行 48 为 **6/9**；线圈金额回答和两项技术报告仍失败，未再次部署、未移动 V1 标签。代码修复仍待完成验收，不能作为已发布版本。
- 原始报告和回滚制品位于 Mac Mini `~/pump-release-staging/d81ff7e/`；本机发布日志、运行 47/48 报告位于 `C:\Users\Dan\Documents\pump-v1-release-archive\2026-09-07`。后续必须先修复并验证剩余失败，再发起新的发布，不重复运行直到偶然全绿。
...
### 2026-09-07 发布失败根因修复与第二次回滚

- 修复正式金额摘要、铜价基数识别、预览总价遗漏、内部工具协议泄露，以及目标缺失/完整空查询的模型反馈与预算结束回答。未增加业务写权限，未恢复旧调度框架。
- 修正评测对单个绕组身份出现在表格后方、正式配方缺失等价证据、缺失结论被误判双重否定的识别；保留错误金额、错误身份、分页截断、接口失败和无关关键词拒绝用例。
- 隔离真实运行 49 为 6/9，50 和 51 为 7/9，原记录不改写。运行 51 保存的原回答/工具回执经最终判定代码只读复核为 9/9；这是离线复核，不是新的真实模型运行。初次启动未就绪导致的一次 fetch failed 单独保留。
- 本地全量测试 1858/1858、契约 26/26、深度 API 437/437、lint 与生产构建通过；修复提交 `51e0425` 已推送。生产完整代码门禁也通过，真实 AI 仅 8/9：零件单价仍误用了模板套件价。失败副本保存在 `~/pump-release-staging/51e0425/`（此数据库的运行 ID 为 47，与第一次失败同号但为不同快照）。
- 已再次恢复 `12fee179b607`、原 Web 构建和本次发布前数据库备份 `pump-release-2026-09-07T09-40-41-330Z.db`；ready=true、migration=79、启动备份通过。逐表对比证明失败期间原有业务表未变，配置未变。
- 补充模板结果价格含义后，隔离运行 52 为 8/9：单价通过，但最后一轮仍输出内部工具协议，被安全拦截。已补齐撤下工具时的查询结束通知，隔离运行 53 为 7/9：报告候选明确排除被误判替换，以及最终轮仍携带历史工具协议。已修正排除候选判定，并在最终回答阶段保留查询数据、移除调用协议。以上失败记录不删除或改写。最终隔离真实运行 **54：9/9**，不是离线重判；本地全量测试 **1860/1860**、lint 通过。 `d553716` 再次生产发布的真实验收为 **8/9**：多次知识查询重复文本累积触发上下文超容。已恢复 `12fee179b607` 和本次备份 `pump-release-2026-09-07T09-52-38-508Z.db`，失败制品位于 `~/pump-release-staging/d553716/`。新增本轮知识去重后，原失败的全部五项工具回执已能离线进入最终回答阶段（不代表真实 AI 通过）。随后真实单项诊断运行 **55：1/1**，本地全量 **1861/1861**、lint 通过；`8b54378` 生产真实验收仍为 **8/9**：唯一失败是判定未识别“没有查询到任何报价记录”，正式查询确为零条，超容与用途检查已通过。已恢复 `12fee179b607` 和备份 `pump-release-2026-09-07T10-01-53-853Z.db`；失败制品保存于 `~/pump-release-staging/8b54378/`。修正零报价表达判定并覆盖疑问、假设、双重否定拒绝路径，原生产回答只读重判 **9/9**；原始 8/9 不改写。日志与只读复核结果保存在上述外部归档目录。
