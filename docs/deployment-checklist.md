...
- 补充模板结果价格含义后，隔离运行 52 为 8/9：单价通过，但最后一轮仍输出内部工具协议，被安全拦截。已补齐撤下工具时的查询结束通知，隔离运行 53 为 7/9：报告候选明确排除被误判替换，以及最终轮仍携带历史工具协议。已修正排除候选判定，并在最终回答阶段保留查询数据、移除调用协议。以上失败记录不删除或改写。最终隔离真实运行 **54：9/9**，不是离线重判；本地全量测试 **1860/1860**、lint 通过。日志与只读复核结果保存在上述外部归档目录。
