...
- 订单“客户要求”页签统一管理原始文件和归纳结果。AI 可按原文归纳并在用户确认后保存草稿，但正式知识必须在订单页面点击“确认进入知识库”；重新编辑不会覆盖上次确认版，撤销确认只移除知识版本，原文件和草稿继续保留。
...
- Web/PWA AI 对话使用统一 SSE 模型适配层，默认 DeepSeek，也可在 `/setup` 切换 Kimi 开放平台。工作台支持上传 PDF、Excel、文本和图片，附件统一保存到 `factory_files` 并随会话历史恢复；文字型 PDF 自动按页解析，Excel/CSV 保留工作表和单元格定位并可生成只读报价映射草稿，图片与扫描 PDF 使用本地中英文 OCR。DeepSeek 可读取 OCR 文字但不接收图片二进制；Kimi 多模态模型可同时接收原图。OCR 技术参数只生成带页码/坐标和置信度的候选，不自动写业务数据。Kimi Coding 订阅凭证与开放平台 API Key 不互通，不能用于业务助手。旧 ASR 接口仅保留兼容，不再由当前工作台调用。
...
- V9.1 增加统一文件对象和 AI 聊天附件。PDF、Excel、CSV、UTF-8 文本和常见图片先经过 10MB 限制、文件名清理、扩展名与真实内容签名核对，再按 SHA-256 保存或复用 `factory_files`；重复上传不会新增二进制对象。知识资料、配方测试报告和 AI 会话附件复用同一文件对象，聊天历史可恢复图片预览和文件下载。模型层默认保持 DeepSeek，支持用环境变量切换 Kimi；只有模型声明视觉能力时才发送图片二进制。
...
- V10.1-V10.2 把客户原始要求和人工确认结论纳入订单追溯。订单详情“客户要求”可上传生产、包装、交付相关 PDF、Excel、图片和文本；附件一键带入 AI 归纳，结果可保存为草稿。只有订单页面人工确认的版本进入订单知识，AI 无权确认；整个流程不自动修改订单、配方、采购或库存。使用范围和后续阶段见 [V10 使用大纲](./v10-user-guide.md)。
