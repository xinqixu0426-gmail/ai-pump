...
- Web AI 对话使用 SSE 流式返回内容，并在工具执行前发送执行计划，标明每一步是只读/试算还是需要确认的写操作。全部 74 个工具的中文 `displayName`、读写、风险、来源、唯一 `executorKey` 和结果 `resultProvenance` 统一由能力注册表提供；能力目录、计划与确认卡片不再维护重复名称，总 executor 也不再按多个领域依次试探。所有工具统一经过 `AI_TOOLS` JSON schema 和 `aiExecutionEvidence`：Query/Preview 必须有本轮正式 API 成功证据；正式查询完成后的“未找到”可如实返回，HTTP/超时/协议失败则停止结论；写工具必须有匹配 capability 的 operation、完成状态和审计 ID，缺一项就按失败处理，确认接口也不得宣称完成。有业务工具时先缓冲模型正文，计划中的必要能力全部通过证据门后才向前端发送结论。历史上下文是否继承由结构化 `contextMode` 决定；新的明确问题不会继承旧写意图。`answerShape` 统一控制直接回答、数量加简报、清单、对比和解释，不再为单个工具增加终止或回答补丁。订单只读查询允许模型把“叶总”等简称扩展为可能的标准客户名，再由正式 API 模糊匹配并要求唯一命中；正式订单 ID 可以在同一轮后续查询中复用，模型直接猜测 ID 仍被拦截。知识库不回答实时库存；能力注册表可以为实体业务查询声明只读知识伴随能力，当前单订单详情、生产准备和处理方案会自动以同一订单目标补充人工确认知识，空记录不扩写、读取失败不作否定结论。
