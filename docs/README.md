...
规格直读命名已有服务端规则、只读名称预览、按 ID 读取现名及受确认保护的历史引用绑定 API，详见 [API 总表](api-reference.md#41-目录命名与引用)。配方库存查询按已保存 ID 读取现名和库存，不依赖弃用旧称；供应商冲突等身份异常与缺货分开显示，原单据快照保留。Web 新建皮垫/配件/包装/其他已接入规格填表和服务端生成校验，零件新增与批量新增可保存命名输入。旧零件直接改名已增加引用和并发保护，有已知引用时暂不允许覆盖名称。所有写入口强制命名、历史引用迁移及全链条实时同步仍在[执行计划](technical-debt.md#material-naming-execution)中推进，当前尚不能视为完整自动同步功能。
