...
配方新建先选择泵壳模板，名称从模板带入并可编辑，不再重复填写命名系列和配置，不自动添加“水泵”前缀；已保存规范配方的改名继续通过正式预览及确认入口，历史名称不批量修改。契约见 [API 接口总表](./api-reference.md)。
...
规格直读命名采用服务端规则，零件和线圈新增按规格生成名称。受保护改名保持原ID，业务读取使用有效ID或经源哈希、规格修订核验的绑定装配现名，保留原始单据JSON、金额和采购进度。配方名称缺少机筒长度时省略该段，对外型号独立保留，后续安全改名不覆盖；Web回读保护编辑草稿。AI当前业务查询在本地、DeepSeek和Kimi中均要求本轮正式取证，历史答案不能替代当前名称或库存；云端保持完整只读工具目录。详见 [目录API](api-reference.md#41-目录命名与引用) 和 [实施状态](technical-debt.md#material-naming-execution)。模板/配方/常用配置新增已强制按规格生成名称，已提供受保护批量迁移及隔离恢复演练；26条明确规格存量的隔离演练与全部目录盘点见[存量名称核对清单](catalog-naming-review.md)；存量含义核对、完整真实环境矩阵和生产切换尚未完成。
...
- **零件**：按型号、分类、供应商记录价格和库存。同型号可以有多个供应商。通用新建要求显式选择分类；“保存并继续”才继承上一条的分类和供应商。包装类使用二级分类：`外包装`（牛皮纸箱、彩印箱、木箱）、`内衬`（泡沫、珍珠棉）和 `固定包材`（说明书、贴纸等）。
...
配方列表把铜价预警和“无铜价快照”统一纳入“铜价待复核”统计与筛选，但仍保留缺快照和数值波动两种不同状态。报价、订单、零件、配方和采购等核心列表首次读取期间统一显示加载占位，不把空数据误报为 0 或“正常”；加载完成后的真实 0 才是业务事实。

表面处理支持：无、喷漆、电泳、电泳+喷塑、整体喷塑和自定义。旧 `paintingWage` 字段只用于历史数据兼容，新逻辑使用 `surfaceTreatmentMode + surfaceTreatmentCost`。

### 报价、订单与采购

- 客户默认利润率按小数保存（`0.10` 表示 `10%`）；报价和直接订单都在显式选择客户后带入并以百分比录入，计算与保存时转换为 `1.10` 倍售价。普通新建不预选客户，系统未配置利润率时交互默认值为 `10%`。
...
- 报价与直接订单以配方保存成本为锁定基线，共用后端受控配置快照，并先受配方自己的配置白名单约束；报价页可改浮球、电缆长度及纸箱/木箱、泡沫、珍珠棉，直接订单页还可改线圈片数、机筒长度和表面处理。包装候选按“型号 + 供应商 + 包材角色”稳定去重并显示正式目录成本价；同型号多供应商时禁止按最低价猜选。线圈变化会同步重算依赖电容；固定 BOM 及目录价格仍由服务端决定，浏览器提交的成本、BOM 或包材快照价不作为事实。
- 组合包材按完整有效清单保存：外包装在纸箱/木箱中选择，泡沫和珍珠棉独立启用，说明书、贴纸等固定包材继续沿用。
- 报价和直接订单界面统一使用利润率百分比（如 10%），保存及价格计算仍兼容历史加价倍数（如 1.10）。
- 新建报价表单通过默认收起、可展开为右侧辅助面板的“询价助手”接收图片、Word、Excel、PDF、CSV 和文本附件；既可选择上传，也可从 WPS 或本地文件夹复制文件后在询价助手区域按 `Ctrl+V` 受控粘贴。多模态模型可生成单文件或最多 4 个来源的联合要求摘要，并独立展示待确认事项，主报价表单保持可操作。创建报价时原件和摘要一并归档，不自动修改正式报价字段。
...
- 未建单的“做 N 台够不够”使用 `POST /api/inventory/virtual-readiness-preview`。它以正式配方/情景 BOM、当前库存和活动订单已占用需求复用订单规划规则，返回库存管理物料的 READY、SHORTAGE 或 INCOMPLETE；调用本身不创建订单、采购清单或库存预留，结果不代表产能、交期或采购可得性。
...
- 直接创建的订单先进入“待确认”；确认后才可登记采购进度。采购中和采购完成由采购数量自动推导。关闭订单必须明确“领用出库并关闭”或“释放库存预留”；前者按订单冻结 BOM 原子扣减零件和正式线圈库存，后者不扣库存且必须填写释放原因。
...
- 采购入库不等于生产领料。订单完成后通过关闭预览核对整张订单的领用扣库明细；确认“领用出库并关闭”才按冻结 BOM 扣库，选择“释放预留”则不扣库并记录原因。订单外库存调整继续走库存页面。
...
- 采购中心按供应商和型号聚合数量进度，可对单物料跨订单整项下单，也可将同一供应商的多种待下单物料一次预览并原子提交；任一任务失败不会部分下单。采购中心本身不执行到货或入库。
...
上述同型号最低价回退只服务于仍需兼容的普通配件估算，不适用于正式包装候选。包装候选使用零件目录的正式单位成本，并按“型号 + 供应商 + 包材角色”稳定识别；同型号多供应商时必须显式选择，不能以最低价猜选。
...
- 电缆和浮球目录中的旧“线径”均表示横截面积（mm²）。电缆各规格价格是元/米，库存按米，成本为 `对应规格每米单价 × 长度 + 配件费`，不再乘面积；电缆和浮球命名只接受截面积 mm²。
...
- AI 回答反馈可以形成结构化纠正规则与回归候选；规则具有作用范围、业务域、类型、规则主题、优先级、生效期、版本和冲突治理，所有新候选必须人工批准后才进入 AI 上下文。纠错规则不进入通用 RAG，防止过期、范围外或冲突规则绕过治理。反馈保存问题、回答和来源快照，删除原会话只隐藏聊天历史，不会删除已经形成的反馈、规则和回归证据。知识库页面可运行并查看当前启用的人工检查；九条旧内置核心系统检查已于 2026-09-19 全部退役（其引用实体随数据库清理不存在），`verify:ai-release` 现在报告“核心用例已退役、本轮没有可执行项”，不再构成 AI 质量证据，恢复需按当前真实实体重新编写用例并做一次显式清单变更，具体见 [AI 学习发布门禁](./ai-learning-release-gate-guide.md)。
...
- 零件、订单、采购及配方工作区（含模板、配置预设列表）通过业务版本轮询回读；配方工作区编辑期间暂缓自动刷新，关闭后补刷，避免目录联动覆盖未保存配置。真实双浏览器验收和完整改名链仍未完成，详见 [命名实施进度](./technical-debt.md#7-p1规格直读命名与全链条引用执行计划)。
- 零件编辑页可点击“检查引用”，通过 `POST /api/parts/:id/rename-impact` 查询引用位置和阻塞原因，分页绑定数据哈希。该报告只读，不授权改名或入库；普通保存仍执行原有保护。
- `node scripts/run-deep-api-smoke.cjs --scope=catalog` 单独验证目录命名新增/回读/保护、只读查询和历史绑定，不执行外部铜价/MCP；不能替代完整深度验收。
