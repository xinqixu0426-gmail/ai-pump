...
- `POST /api/orders/:id/purchase-items/progress` 支持部分采购、部分到货和分批入库；只有入库增量会在同一事务内增加零件库存，并写入采购项批次历史。
...
- AI 知识回答在正文下方显示可追溯的“回答依据”，来源随会话消息一同保存，历史会话也能回看。知识来源可以打开知识详情或原业务页面；待同步条目会显示警告。普通价格、库存、订单和成本查询标记为“实时业务数据”，与“知识库快照”明确区分。
...
- Knowledge Base V2 第一阶段在 V1 事实检索之上增加配方智能检查。配方编辑页可手动检查当前未保存 BOM 草稿，点击保存时也会自动检查；系统按模板、BOM 角色、具体型号和线圈配置寻找相似配方，区分“配置字段与 BOM 的确定性矛盾”和“同类配方高频项复核建议”，并对可比固定件给出带来源的价格提醒。高置信度问题会在落库前要求返回修改或明确确认后继续，普通复核建议不阻止保存；动态泵壳、线圈、浮球、成品电缆和公式项不会套用普通零件价格规则。
...
- 管理看板“知识库回归检查”首批覆盖价格更新、线圈多方案、性能测试报告类型、测试模板无效字段、客户报价顺序和成品电缆语义。运行会调用外部 AI，因此不并入普通 `npm test`；由管理页面手动执行并保存每次结果，业务数据库不会被修改。
