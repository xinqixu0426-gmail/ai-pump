...
- 通用 MCP：默认关闭，以每 Agent 独立 Bearer service token 提供固定只读 Query/Preview 白名单；同一 `/mcp` 同时服务 2026 协议客户端和 Hermes 使用的 2025 Streamable HTTP，任何 Agent 都不接触 `INTERNAL_SECRET`。开发、兼容和一致性门禁见 [mcp-development-guide.md](./mcp-development-guide.md)。
...
| 通用 MCP | `/mcp` | 远程无状态 Streamable HTTP；把 12 个已登记只读 AI capability 提供给 Hermes、Codex 等兼容 Agent，双协议协商且仍由 executor 调用正式 API，不开放写工具 |
