...
- 报价转订单先通过 `POST /api/quotations/:id/order-draft` 预览，再调用 `POST /api/quotations/:id/convert` 在同一事务内创建订单、关联报价并更新状态，重复转单会被拒绝。
- 订单保存前通过 `POST /api/orders/save-payload-draft` 统一生成 `itemsJson`、`purchaseListJson` 和 `todosJson`。
- 订单状态、采购项勾选、待办勾选和“确认采购完成并入库”必须通过订单动作 API 执行；确认入库在后端事务内先校验全部零件，再同时更新库存、订单和唯一入库凭证，重复调用不会重复增加库存。
- 采购中心按供应商和型号聚合后，通过 `POST /api/orders/purchase-items/batch` 批量标记未完成订单的采购项；采购中心不入库。
- 订单保存产品、数量、单位成本、售价、采购清单和待办快照。
- 采购清单按 `partId` 或“型号 + 供应商”汇总，并在全部活动订单间顺序分配当前库存，避免多个订单重复占用同一库存；库存入库和人工调整通过订单动作或零件库存接口更新。
