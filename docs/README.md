...
| [API 架构审核与解耦报告](./api-architecture-audit.md) | 保存 214 个路由的能力矩阵、风险、整改状态和 P0/P1/P2 清单 | 架构状态或优先级变化 | 阶段性审计 |

接口事实不在 SOP 重复，永久规则不在接口总表重复，风险与重构建议只在审核报告维护。
...
| 架构 | [模块化设计](./modularization-design.md)、[数据库结构](./database-schema.md) | 模块化单体演进和 SQLite Schema/迁移规则 |
...
- 客户报价和订单历史由只读 `GET /api/customers/:id/context` 从正式业务表实时聚合；AI 只负责按名称定位客户并调用该接口，不在 executor 内复制筛选、排序或历史拼接规则。
...
职责、事实来源、事务、幂等、AI 权限和兼容/废弃建议见
[API 架构审核与解耦报告](./api-architecture-audit.md)。
...
- AI 修改零件时，型号定位、字段差异和正式 API 调用由 `aiPartExecution` 编排，executor 只委托。元数据必须走版本化 `/api/parts/:id`，库存必须走 `/api/parts/batch-stock-preview` → `/batch-stock`，AI 不直接写库或把库存塞回普通 PATCH。一次 `update_part` 只能修改资料或调整库存；混合请求在确认前拒绝并拆成两个分别确认的建议操作，避免跨 Command 部分成功。
...
本节只保留仍然生效的系统边界。接口字段和路径以 [API 接口总表](./api-reference.md) 为准，架构风险以 [API 架构审核](./api-architecture-audit.md) 为准，阶段实现过程不再写入当前说明。
