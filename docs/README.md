...
配方页以“泵壳模板 + 线圈配置 + 客户选配”为唯一可见主流程：泵壳模板提供结构成本包和客户可选配置范围，新建配方复制规则后可独立调整；线圈配置联动电容、电缆/浮球线径和叶轮参考，客户选配再覆盖电缆长度、浮球、包装材料、接轴和表面处理等 OEM 差异。历史未配置范围的模板和配方继续按开放模式运行，既有报价、订单和成本快照不被回写。
