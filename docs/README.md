...
规格直读命名已有服务端规则、只读名称预览、按 ID 读取现名及受确认保护的历史引用绑定 API，详见 [API 总表](api-reference.md#41-目录命名与引用)。配方库存查询已区分身份异常与缺货，并显示目录现名和保存时型号。Web 新建皮垫/配件/包装/其他已接入规格填表和服务端生成校验，零件新增与批量新增可保存命名输入。所有写入口强制命名、历史引用迁移及全链条实时同步仍在[执行计划](technical-debt.md#material-naming-execution)中推进，当前尚不能视为完整自动同步功能。
