...
- V9 文件智能处理将提供两种等价入口：在客户、报价、配方或技术档案页面上传，系统天然知道关联对象；也可以把 PDF、Excel、图片或报价文件直接发给 AI，再由 AI 询问是仅临时分析，还是保存并关联到具体业务对象。需要长期检索的文件必须保存原件、解析结果、来源关系和处理状态后才能进入知识库，聊天附件本身不能作为唯一存档。
