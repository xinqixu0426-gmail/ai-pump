...
规格直读命名采用服务端规则，零件和线圈新增按规格生成名称。受保护改名保持原ID，业务读取使用有效ID或经源哈希、规格修订核验的绑定装配现名，保留原始单据JSON、金额和采购进度。配方名称缺少机筒长度时省略该段，对外型号独立保留，后续安全改名不覆盖；Web回读保护编辑草稿。详见 [目录API](api-reference.md#41-目录命名与引用) 和 [实施状态](technical-debt.md#material-naming-execution)。本批代码已收口，模板/配方/配置新增入口强制命名、存量批量迁移和生产切换尚未完成。
