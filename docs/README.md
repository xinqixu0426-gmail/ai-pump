...
- `GET /api/workbench/action-center` 把订单准备、经营风险、数据质量、规则学习和知识库健康合并为统一“今日待办”，按优先级给出建议动作和业务页面入口；看板与 AI 共用该只读结果，不自动执行任何处理动作。当前按单人管理助理设计，不要求分配负责人。V7.1 记录生命周期，V7.2 生成最多 3 项可解释的今日执行队列，V7.3 再提供最短处理路径和完成标准；只有现有订单安全动作能交给 AI 发起确认，查看过程仍不写库。
