...
规格直读命名已有服务端规则、只读名称预览和按 ID 读取现名的正式 API，详见 [API 总表](api-reference.md#41-目录命名与引用)。所有写入口强制命名、历史引用迁移及全链条实时同步仍在[执行计划](technical-debt.md#material-naming-execution)中推进，当前尚不能视为完整自动同步功能。
