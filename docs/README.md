...
- V10.1 开始把客户原始要求纳入订单追溯。订单详情新增“客户要求”，可上传生产、包装、交付相关 PDF、Excel、图片和文本；文件以 `order + customer_requirement` 关系保存。附件可一键带入 AI 工作台生成分类草稿，但当前仍由人工确认和实施，不自动修改订单、配方、采购或库存。使用范围和后续阶段见 [V10 使用大纲](./v10-user-guide.md)。
