...
- 管理看板“知识库”视图用于观察 Knowledge Base 的实际状态，不增加独立导航。该视图提供全文搜索、知识分类、最新/待同步筛选、来源追溯和知识详情，并通过只读内容哈希比较识别待新增、更新及来源已删除条目。V4 默认在核心业务写入后自动刷新派生知识；人工二次确认同步仍可执行全量核对和故障恢复。
