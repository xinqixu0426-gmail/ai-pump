...
- 客户新增、修改和软删除保留原 URL，统一由 `customerCommands` 执行；Web 新调用使用持久幂等，修改/删除携带 `expectedUpdatedAt`。删除客户不会级联删除历史报价，存在关联报价时在命令回执中提示保留关系。
...
- 报价保存前通过 `POST /api/quotations/save-payload-draft` 由后端重新试算，统一生成含完整 BOM/成本快照的 `itemsJson`、`totalCost`、`totalPrice`、`previewHash` 和建议幂等键。新增/修改必须复用该预览，服务端保存时再次调用正式成本逻辑，不信任浏览器提交的成本和合计金额。
...
- 报价转订单先通过 `POST /api/quotations/:id/order-draft` 预览并取得报价版本、确认内容哈希和建议幂等键，再调用 `POST /api/quotations/:id/convert`。执行前会重算订单及采购清单，其他活动订单或库存变化导致预览漂移时要求重新确认；建单、报价关联、持久化 operation 回执和两条强审计在同一事务内提交。
...
- 采购入库不等于生产领料，订单关闭不会自动扣库存，实际领用继续走库存页面人工出库。
...
- AI、工厂配置、语音：JWT Cookie 或内部 Secret。
- Siri：`x-siri-token`；生产环境必须配置 `SIRI_API_TOKEN`。
- 登录限流：每个 IP 每分钟最多 5 次。
- 外部市场、语音、CAD 和内部 API 请求统一设置超时；只有幂等 GET 可按策略有限重试。
...
| AI/语音/Siri | `/api/ai`、`/api/voice`、`/api/siri` | 对话、工具调用、ASR；模型流解析与工具消息协议由独立 service 统一 |
...
- AI 调度器 V2 先让模型通过强制结构化协议提交当前目标、`conversation/query/analysis/command` 模式、业务域、上下文依赖、回答形式、歧义和最小能力步骤，再从能力注册表自动生成本轮工具 allowlist；执行轮按模型自己的步骤顺序一次只开放当前能力，计划完成后才开放同域补充能力。普通业务轮最多开放 18 个相关工具，非 `command` 轮不开放写工具，纯闲聊不发送业务工具。DeepSeek V4 Flash 调用显式关闭 Thinking 模式，意图识别和结果提取使用非思考模式；provider 仍保留不支持 `tool_choice` 时的协议级自动降级。调度不再使用业务关键词正则、确定性预取或全量工具回退；新增能力完成注册表、唯一 JSON schema 和 executor/API 映射后即可接入，无需修改调度分支。
...
- AI 调度器 V2 将普通工具结果作为模型继续编排和提取结论的证据；证据以“不可信业务数据”角色进入最终合成，字段中的提示词或命令不会取得系统权限。意图信封要求澄清时必须没有执行步骤，服务端直接停止工具链并向用户列出缺失信息。只有目标明确后的写操作确认会中断等待用户确认。每轮只记录规划、合成、总耗时、工具数、供应商路由与结果状态，不记录用户或附件正文。SSE 与 Siri 的非流式入口共用同一个 `aiDispatcherV2`，不再维护两套循环。
- V2 最终提取按 `answerShape` 只返回当前问题的结果：内部 ID/sourceId/数据库序号默认不展示，列表不由模型自行计算分组数量，也不附加未询问的库存风险、相似项判断和后续建议。
- Web AI 对话使用 SSE 流式返回内容，并在工具执行前发送执行计划，标明每一步是只读/试算还是需要确认的写操作。全部 74 个工具的中文 `displayName`、读写、风险、来源、唯一 `executorKey` 和结果 `resultProvenance` 统一由能力注册表提供；能力目录、计划与确认卡片不再维护重复名称，总 executor 也不再按多个领域依次试探。所有工具统一经过 `AI_TOOLS` JSON schema 和 `aiExecutionEvidence`：Query/Preview 必须有本轮正式 API 成功证据；正式查询完成后的“未找到”可如实返回，HTTP/超时/协议失败则停止结论；写工具必须有匹配 capability 的 operation、完成状态和审计 ID，缺一项就按失败处理，确认接口也不得宣称完成。有业务工具时先缓冲模型正文，计划中的必要能力全部通过证据门后才向前端发送结论。历史上下文是否继承由结构化 `contextMode` 决定；新的明确问题不会继承旧写意图。`answerShape` 统一控制直接回答、数量加简报、清单、对比和解释，不再为单个工具增加终止或回答补丁。知识库只处理用途、经验、规则依据、历史确认知识或明确知识查询，不回答实时库存。
...
- AI 按类别批量调价时，同一 `aiPartExecution` 只生成逐项候选价，再强制调用 `/api/parts/prices-preview` → `/prices`；服务端预览负责正式价格校验、资源版本和预览哈希，执行端负责整批事务、持久化幂等和逐项强审计。百分比/固定金额换算尚未成为通用业务 API，其他调用方不得复制该规则。
...
- 已保存的 AI 回复支持标记“准确”或报告“内容错误、来源过期、资料不足”。反馈会保存当时的问答和知识来源快照，问题进入管理看板“知识库”的待处理队列。报告内容错误时可填写正确做法并选择“让 AI 长期记住”，系统会生成可启停的全局纠正规则，在后续相关问题中优先加载并进入知识规则索引；来源过期、资料不足和未明确勾选的反馈不会自动学习。规则只约束 AI，不会改写订单、库存、配方、成本等业务数据。
...
- `/voice` 页面已弃用并兼容跳转到 `/ai`；旧基础语音助手不再作为产品入口。`POST /api/voice/asr` 仍由微信小程序语音页调用，因此作为受内部密钥保护的外部兼容接口保留。
...
- Siri 使用快捷指令文字输入，不经过 ASR；统一调用 `POST /api/siri/chat`，由 AI tools 决定业务动作，Siri 不直接访问库存、BOM、采购等内部 API。它是独立的快捷指令集成，不是已弃用 Web Voice 的重复入口。
- Siri 返回 `speech` 供朗读，内容保持简短；结构化结果通过 `resultUrl` 查看，结果临时保存在内存中，5 分钟后失效。
- Siri 写操作返回 `confirmation_required`、`confirmationId` 和确认摘要；用户明确确认后再调用 `POST /api/siri/confirm`。外部字段保持兼容，服务端内部已复用与 Web 相同的单次 `confirmationToken`、标准 API 和审计路径。
- Web/PWA AI 对话使用统一 SSE 模型适配层，`/setup` 默认启用智能路由：普通对话走 DeepSeek，统一文件库确认的图片原图、PDF、Excel/CSV 和文本附件自动走 Kimi 开放平台 `kimi-k3`；图片直接使用 base64 原图，非图片文件通过临时上传和 `file-extract` 抽取后交给 K3。K3 成功看原图时不重复附加整段 OCR；Kimi 请求失败时自动回退 DeepSeek 与本地解析/OCR。供应商请求建立失败、429 和 5xx 最多重试 3 次；响应流中途断开统一返回可诊断、可重试的网络错误码，由 Web/PWA 重新请求完整回复。每条 AI 回复显示实际使用的提供商。工作台附件统一保存到 `factory_files` 并随会话历史恢复；本地文字型 PDF、Excel/CSV、图片和扫描 PDF 解析继续作为持久化检索与故障兜底。OCR 技术参数只生成带页码/坐标和置信度的候选，不自动写业务数据。Kimi Coding Plan 凭证与开放平台 API Key 不互通，不能用于业务助手。旧 ASR 接口仅保留兼容，不再由当前工作台调用。助手最终回复只呈现面向用户的结果，不展示内部思考、逐步推理、工具选择或处理过程；简单问题使用短段落，一般问题可使用一个简短标题和 2-5 个短要点，保留结论、关键数字或异常、必要下一步和风险。型号或规格中的 Markdown 特殊字符使用行内代码保护，处理过程与依据由正文上方的折叠区承载。
...
- 业务附件关联不等于知识入库。归档到客户、报价、配方、质量证据或知识资料必须使用正式文件能力，引用保护阻止误删已确认事实的来源文件。
...
- `/voice` 仅兼容跳转到 `/ai`；`POST /api/voice/asr` 仍供微信小程序使用。`model-variants` 仍有页面和测试调用方，两者均不得在缺少调用证据时删除。
