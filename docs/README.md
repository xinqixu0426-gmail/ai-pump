...
| 通用 MCP | `/mcp` | 远程无状态 Streamable HTTP；默认向 Hermes、Codex 等兼容 Agent 提供 45 个只读工具，显式授权的 2026 客户端可额外使用 17 个带原生人工确认的写工具；全部仍由 executor 调用正式 API |
...
- 通用 MCP 从同一 capability registry 和 `AI_TOOLS` schema 生成 45 项只读白名单及 17 项显式审核写白名单，通过统一 executor、internal API client 与执行证据门读取或变更正式事实。Bearer service token 只用于 `/mcp`，不能调用普通 `/api/*`；写能力需独立开关、身份 allowlist、正式 Preview、HMAC 绑定的多轮状态和 MCP form elicitation 人工确认。官方 SDK v2 使用一个 server factory 同时处理 2026-07-28 与 2025 版协议；旧客户端不支持交互确认时只读能力不受影响，写操作安全拒绝。
