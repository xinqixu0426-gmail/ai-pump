...
- 候选规则经人工批准后立即参与相同泵壳模板的配方检查，不必等待知识库同步；同步只负责让 AI 检索到该规则。规则提醒会显示批准记录和证据配方，客户定制差异可标记为特殊情况，系统不会自动添加零件或修改成本。
- Knowledge Base V2 第二阶段增加候选业务规则审核。系统只从人工明确确认的同类高频项中学习，按泵壳模板和提醒键归纳，至少需要两个不同配方作为证据；价格提醒和确定性程序错误不会生成候选规则。候选项在看板“数据质量”中批准或驳回，只有已批准规则会在下一次知识库同步时进入 `business_rule` 检索，系统不会自动批准或自动修改配方。
