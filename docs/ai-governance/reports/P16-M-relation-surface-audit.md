...
## Current isolated release checkpoint — 2026-09-07

**MANUAL_ACCEPTANCE_READY_WITH_PRODUCTION_DATA_LIMITS.** Isolated owner Candidate revision `c98dd7efb7c43f33f705ae9095f356094f5fa829` is running from `/Users/dan/pump-p16m-choice-20260907/source` with strict boolean `multiReadEnabled:true`. Source default remains OFF. Legacy revision `12fee179b6074215cf359bcc1a789ce1a345b9ba`, PID59155, source, frontend build, owner-default routing and business DB hash/mtime/size/backup count are unchanged. Final Candidate PID83313 is ready. No Legacy signal, regular deployment script, production business write, P17 reactivation or P16-N work occurred. Code was committed through an isolated index; no push was made and the user's other dirty files and documentation hunks remain uncommitted.

Production has **one customer**, so customer multi-page selection is **locally certified, production-inapplicable with current data**, not production-passed. A bounded formal read of the14 coil profiles supplied scheme codes omitted by the initial name/lookup-only preflight; a separate real two-identity coil cohort then passed4/4. No data was created to manufacture acceptance. Full scenario production certification remains incomplete for customer paging: `P16_M_PRODUCTION_COMPLETE=NO`, `P16_N_READY=NO`, `P17=PAUSED`. This is a running version ready for manual acceptance, not a claim that every production scenario has been exercised.

| Check | Actual result |
|---|---|
| Clean, task-only artifact tests |2289/2289 PASS; excludes the user's unrelated Legacy/shadow changes and tests |
| API contract / deep API / build |26/26 PASS;437/437 PASS, copied-DB integrityOK/FK0; buildPASS |
| Current task lint / dependencies |Scoped lintPASS; root/web dependency audits0. Only compatible transitive fast-uri3.1.7 and qs6.16.0 updated; new Candidate uses independent dependencies |
| Full conventional release command |Not green:14 pre-existing unused-variable errors in12 byte-equivalent baseline files remain. Local development env lacks CORS_ORIGIN; actual production env validates. Do not report the conventional release command as PASS |
| Existing real-model local evidence |Choice18/18 and existing read60/60; current implementation hashes still match. Old failed/cohort observations remain unchanged |
| Actual public owner SSE reads |18/18 PASS: seven relations, continuation, lists, ordinal/exact details, count, price, part/coil inventory and recipe cost; formal oracle/evidence validated |
| Public owner customer-choice flow |5/5 PASS: keyword candidates, explicit single selection, verified missing customer, contextual surname, resumed original customer.orders query |
| Public owner coil-choice flow |4/4 PASS using two explicit exact identities from formal profiles: advertise/choose second, advertise/choose both; exact formal detail oracles, at most2 Tool calls |
| Owner isolation / mixed writes |Shared/anonymous excluded from Candidate; Candidate-only mixed-write negative has0 Tool/answer calls |
| Invalid formal parameters |4/4 rejected with400: keyword type, resource mismatch, pageSize51, negative cursor |
| New conversation / forged context |New-conversation ordinal:0 Tool calls and no delivery; normal risk model correctly runs without certified state. Invalid signed context:HTTP200 SSE error,0 Candidate executions,0 content events |
| Actual rollback and reapply |Both switched isolated Candidate/Gateway roles and passed public owner read smokes; original stable Candidate was restored before the final new revision was reapplied |

Validation failures were retained and diagnosed rather than erased. The clean directory initially lacked the DB/backup fixture required by an import-safety test, byte-frozen files had checkout line-ending differences, and the first test wrapper incorrectly injected application credentials into unit tests. Correcting only this verification setup produced the clean test result above. The deep-API copperBase race recurred: three equal reads before an asynchronous startup fetch finished did not establish stability. The test runner now waits for the startup copper-sync terminal event before freezing the formal oracle; runtime price logic and Legacy were not changed. Expected negative HTTP logs in the deep suite are not test failures.

The first production cohort's18 positive reads all passed, but candidate preflight assumed multiple customers. A focused follow-up then incorrectly attempted an absent next-page cursor; the formal API rejected it before the next model request. Both scripts restored stable Candidate and their REWORK records remain intact. The completed customer cohort records single-customer and then-observed no-two-coil applicability. The latter was superseded by the broader formal-profile preflight and separate4/4 coil evidence, not by rewriting that record. The original boundary oracle also incorrectly expected no risk call for an unbound ordinal and HTTP4xx for the existing SSE rejection protocol. Its original REWORK is retained; contract review confirms the former's0-read/no-delivery safety, and one separate invalid-context transport check confirms rejection before execution. No business-runtime change or model retry was added to satisfy these harness assumptions; the18 passed production read questions were not replayed as another full cohort.

Evidence: [release summary](../data/p16m-choice-release-summary.json), [initial production cohort](../data/p16m-choice-production-initial.json), [retained focused harness stop](../data/p16m-choice-production-choice-harness-stop.json), [completed customer-choice cohort](../data/p16m-choice-production-choice-final.json), [coil-choice cohort](../data/p16m-choice-production-coil.json), [original boundary observation](../data/p16m-choice-production-boundary-original.json), [context rejection](../data/p16m-choice-production-context-rejection.json), [rollback](../data/p16m-choice-production-rollback.json), [reapply](../data/p16m-choice-production-reapply.json). Evidence contains metadata and hashes, no credentials, questions, answer bodies or business rows. Production observations have0 orphan/cross-request spans and0 private-value leakage. Historical M-24 RISK_UNAVAILABLE remains the owner's non-blocking unexplained occasional failure; old29/30 is not rewritten as30/30.

Manual acceptance can begin in the existing owner AI conversation: `查找姓张的客户` (zero or bounded candidates must be stated honestly), `有哪些客户` then `第1个`, or `客户「候选验收不存在」有哪些订单？` then the actual customer's surname and `第1个` (must resume that customer's orders, never silently replace the target). `哪些零件库存不足？` then `继续` exercises live paging. Two-coil prompts can use the two exact identities found in formal profiles; `第2个` / `两个都看` may only read the displayed candidates. The user-facing handoff provides copy-ready current labels separately from metadata-only certification artifacts. The local fixture cohort additionally certifies expiry/cross-principal/cross-session/tamper/API-failure/mixed-write boundaries without production mutations.

All remaining sections below are historical implementation checkpoints and retain their original version-specific results.
