...
配方工作台的“电缆试算”是独立只读小工具，选择具体供应商电缆线材、长度和配件类型即可使用，不绑定模板、不创建配方。复用正式 `/api/recipes/bom-draft` 动态 BOM 预览，展示成品电缆正式金额与公式；缺价及错误明确显示，输入变更不接纳旧请求结果。新建配方未选模板时预览传 `templateId=null`，不传非法的 `0`。

配方浮球/电缆实时成本直接展示正式 BOM 预览中的对应物料，不再按目录型号重复猜测。横截面、供应商物料 ID、长度及配件类型均参与预览更新；首次请求显示计算状态，失败显示正式错误，不能持续以“等待生成成本”掩盖失败。价格仍由正式服务计算。

配方详情提供“打印配方”，使用 A4 竖版、9mm 页边距和紧凑明细，沿用网页版的圆角成本卡片、浅色表头及费用卡片。输出保存快照 BOM、费用（含工艺）及关键/技术参数，保留杂项分组及逐项明细。型号列增宽、金额列紧凑，优先让常规 BOM 落在一页；超长明细自然分页，不截断、不隐藏数据。打印期间网页标题使用配方名，作为浏览器保存 PDF 的默认文件名；关闭详情恢复原标题。浏览器打印时隐藏应用和库存，跨页重复表头，明细不使用滚动容器；取消打印不影响业务数据。建议使用 100% 缩放并关闭浏览器页眉页脚，系统对话框仍可覆盖文件名、纸型和缩放。

配方详情 BOM 的线圈型号展示“规格 / 片数 / 材质 / 槽眼 / 线重”，优先读取保存的 BOM 参数，缺失时使用配方已保存参数；没有记录的参数明确标注未记录，不使用方案名冒充型号，也不读取当前目录参数覆盖快照。

配方详情的 BOM 位于关键参数前，自然铺开，不设置内部纵向限高滚动。单价不超过 1 元的螺丝及皮垫/垫圈使用小分组标题，下方另起行缩进展示各项名称、数量、单价和小计，不合并明细。分组按快照小计合计与其他单项统一降序，组内按快照小计降序（缺价置后）。人工、表面处理及管理费展示已有保存回执，表面处理显示具体工艺，完整金额以正式保存成本为准。配件与线圈库存默认折叠，可手动展开及刷新。
