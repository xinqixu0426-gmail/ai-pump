...
横向字段行必须对齐输入控件本体，而不是包含说明或错误信息的整个 `Field` 盒子。同行存在 `hint` / `error` 时，不得在父级直接用 `items-end` 对齐所有 Field；字段保持顶部自然对齐，操作按钮用自身外边距和 `self-start` 对齐控件底边。同行混用单行 `Input`、`Select`、`Textarea` 时必须使用一致的明确高度；允许调整高度的文本区与单行控件同行时使用顶部对齐，避免文本区拉高后再次拖动其他控件。多行文本区只有在业务确实需要时才允许更高或由用户调整。

允许自由输入并提供目录候选的字段统一使用 `EditableValueSelect`：焦点或点击展开全部候选，输入时按型号及候选标签筛选，忽略大小写并支持空格分隔的多关键词；清空恢复全部，无匹配时明确提示且允许自定义输入。`ArrowUp` / `ArrowDown` 移动筛选后的活动项，`Enter` 选中，`Escape` 先关闭候选且不得联动关闭父弹层；连续使用 `Backspace` / `Delete` 编辑时同步更新筛选，不阻断输入。输入焦点保留在 `combobox` 上，通过 `aria-activedescendant` 关联当前候选，鼠标点击和键盘选择必须走同一个 `onChange` 业务入口。
...
配方编辑器仅在打开时定位首个待完善分区；录入、添加物料或成本状态变为完成时只更新徽标，不自动折叠或切换分区。分区展开/收起及状态导航由用户主动操作，避免浮球、电缆及多个包装/选配件连续录入被打断。

文本候选框的检索文字暂存在组件中，不在逐字输入时更新正式物料和成本草稿；选择候选才写入其原始 value，自定义文字通过 Enter 或失焦确认，Escape 放弃未确认文字。数字候选框保持即时更新。

`EditableValueSelect` 的候选列表通过 Portal 挂载到 body，使用 fixed 定位，避免表格或包装材料滚动容器裁切。滚动及窗口调整时更新位置，可视区域下方不足时向上展开；候选点击保持输入焦点，外部点击关闭列表。
