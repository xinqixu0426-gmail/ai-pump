...
| 泵壳模板 | 保存泵壳套件或自由搭配规则、人工工资、表面处理工艺和转子默认参数；套件模式引用零件库整套泵壳，自由搭配的组合名称可手输或选择泵壳型号，单件和供应商小套件父型号只能绑定零件库“泵壳搭配”分类；小套件父项按供应商报价整体计价和管理库存，内部组成仅作结构说明；机筒从铝机筒、不锈钢拉伸筒、铁机筒中选择，只有独立的不锈钢拉伸筒按长度动态计价 | 不代表最终产品 BOM；自由搭配模板名不承担库存；小套件组成项不单独计价、扣库存或参与零价检查 |
