...
`用户表达 → 第一阶段目标/风险/业务域信封 → 第二阶段信封内能力步骤 → 能力图 → Agent 调查循环（假设 → tool schema → 正式 API → 结构化观察 → 调整）→ 正式实体绑定 → 证据门 → 结果提取`

- 模型负责口语、简称、疑问、否定和跨领域目标的语义理解；禁止为业务意图继续添加关键词正则。正则只处理 ID、数值、单位、token 和传输协议等确定性语法。
- 第一阶段模型只看到精简业务域目录并提交结构化目标、读写风险、业务域、上下文来源、回答形态、对象范围和歧义；第二阶段只看到该信封内最多 24 项相关能力，并只提交最多 5 个起始事实步骤。服务端继承第一阶段字段并拒绝未下发、越域或读写模式不符的步骤；模型不得直接选择 URL，也不得通过多领域能力扩大第一阶段信封。
- 正常执行最多开放第二阶段步骤关联的 18 项工具；正式 Query 返回零结果、歧义或可恢复的对象未命中时，Agent 可以在限定轮次、调用数量和第一阶段业务域信封内使用注册表声明的只读 discovery/query 能力调整调查策略。写能力、跨域能力和未登记能力不得因恢复调查而扩大。
...
- 每次提供商请求必须在统一输入预算中同时核算系统提示、历史消息、工具 schema、tool choice、附件片段、图片预留和正式证据；分项上限不能相加后突破模型总上下文。预算不足时必须明确超限，不能静默删除正式业务字段。
- 长附件和知识正文按当前问题选择带字符范围的相关片段；知识列表不得继续附带完整正文。运行监控只把供应商明确返回的 usage 记作真实 token，用本地估算控制上下文但不得冒充计费数据。
