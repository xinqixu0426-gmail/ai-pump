...
`用户表达 → 业务域/意图识别 → 类型化查询条件 → schema 校验与规范化 → 执行计划 → 正式 API → 结构化结果`

- 规则编译器负责确定性业务表达；模型生成的工具参数只能作为候选参数，也必须经过同一校验器。
- 执行计划展示规范化后的条件以及参数来源、校验状态，不展示未经校验的模型猜测。
...
- 无法可靠映射到契约字段的文本不得降级成宽泛关键词；应忽略无业务含义的语气词，或明确要求补充必要条件。
