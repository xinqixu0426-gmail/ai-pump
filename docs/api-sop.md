...
`用户表达 → 模型结构化意图信封 → 能力目录/最小步骤 → tool schema 校验 → 正式 API → 证据门 → 结果提取`

- 模型负责口语、简称、疑问、否定和跨领域目标的语义理解；禁止为业务意图继续添加关键词正则。正则只处理 ID、数值、单位、token 和传输协议等确定性语法。
- 模型先提交结构化意图和最小能力步骤，再从能力注册表派生的本轮 allowlist 中生成 tool call；不得直接选择 URL。
- 执行阶段按模型计划顺序一次只开放当前能力；完成计划后才可开放同域补充能力。新增能力不得增加按业务句式分流的执行分支。
- 能力描述必须说明权威职责和排除项；List/Query、Calculation/Preview、Knowledge 不得职责重叠。新增能力若与现有能力相近，应先收口边界或替换旧能力，不能把选择冲突留给提示词碰运气。
- 每个 tool call 直接使用 `AI_TOOLS` 中的唯一 JSON schema 校验，禁止另建自然语言删词或参数重写器。执行计划只展示通过校验的参数，拒绝项不得执行。
- API 是事实与过滤语义的权威来源；AI 不得在返回后自行发明另一套筛选口径。
- 历史对话只提供语言上下文，不提供执行授权。新的明确业务问题只按当前用户轮次分类；跨业务切换必须清除旧写意图，只有紧邻的指代追问或缺参补充可以继承上一轮。模型 tool call 还必须经过“本轮实际下发 allowlist + 当前读写意图”双重执行前校验。
...
- 正式 API 已完成但没有匹配对象时记录为“已验证负结果”，允许如实返回未找到；网络、超时、HTTP、协议或中途调用失败记录为“未验证失败”，必须停止业务结论。
