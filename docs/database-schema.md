...
- `business_change_events` 以 `operation_id` 唯一保存完成命令的业务域、事件类型、安全摘要、原因、结构化变化、审计 ID 快照、详情引用、操作者和时间；`business_change_event_entities` 保存一个跨资源 operation 影响的所有已登记正式业务领域对象。领域清单以能力注册表和 API 契约为权威，数据库用字符串保存而不维护第二份枚举。两表只允许追加，数据库触发器拒绝更新和删除；事件不外键依赖默认 90 天清理的 `api_operations`。
