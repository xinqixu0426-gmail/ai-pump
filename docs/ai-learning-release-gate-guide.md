...
发布门禁与页面手动运行仍是两次独立记录，但共用核心检查基线：9 条已启用内置系统检查会阻止错误 AI 版本通过发布验收；已批准、已启用且打开发布门禁的用户纠错案例在此基础上继续追加。
...
- 9 条核心系统案例任一缺失、停用或未进入门禁：发布失败，不能记录为 skipped 后继续。
- 模板配置成本案例不仅核对工具参数、正式 BOM 和总成本；最终回答还必须明确包含完整模板、`12-120`、带浮球、木箱、珍珠棉和正式总成本，不能只报金额而省略用户指定条件。
...
涉及模板配置成本能力的改动，除固定核心门禁外，还必须用本次缺陷的真实问句做专项验收，确认工具链包含正式模板解析和 `build_recipe_bom_draft`，回答成本等于响应中的 `costPreview.currentTotalCost`，包装角色没有遗漏或重复，并在最终回答中明确复述完整模板、`12-120`、带浮球、木箱和珍珠棉。工具内部参数正确但回答遗漏配置条件同样视为失败。专项验收是代码任务证据，不替代固定核心发布门禁。
