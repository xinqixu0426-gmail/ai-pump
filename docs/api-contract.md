...
11. 零件、线圈、订单、采购、配方等自然语言事实查询必须先识别业务域、意图和实体目标，再编译为类型化查询字段并通过统一校验器；型号、配方名等实体必须剥离语义标签与事实字段描述，禁止把整句或删词后的任意剩余文本直接作为关键词。
12. 规则编译结果和模型生成的工具参数必须经过同一查询契约。未知字段、无意义关键词、非法枚举和越界数值必须在调用正式 API 前拒绝。
13. 同一只读问题要求“明细和当前成本”等多个事实时，必须由一个确定性的组合查询能力编排对应正式 Query/Preview API；保存快照、知识快照和模型常识不得代替当前事实。
14. 执行计划只能展示规范化后的参数，并标明参数来源和校验状态；被拒绝的模型候选不得执行。
15. 正式 Query API 持有筛选语义；AI executor 只负责传递已校验参数和压缩展示结果，不得在 API 返回后另行筛选或重算。
16. AI 的历史消息窗口只能用于语言理解，不能作为可执行状态。新的明确业务问题必须以当前用户轮次重新判定领域和读写意图；跨零件、线圈、配方、模板、订单、采购、报价等业务切换时，不得继承更早轮次的写意图、参数、确认或成功结论。只有明确依赖上文的紧邻追问或缺参补充可以继承上一轮语义；“不要提到被忽略字段”“尚未执行”等否定描述中的写词不得升级为写意图。
17. 模型返回的每个 tool call 必须同时属于服务端本轮实际下发的工具 allowlist，并符合当前轮次的读写意图；不满足任一条件时统一执行器必须在调用正式 API 或签发 confirmation token 前拒绝。历史写入指令、模型自行重放和并发旧请求均不能扩大本轮权限。
18. 查询结果必须返回已应用的结构化条件；零结果与读取失败必须是可区分的结果。
