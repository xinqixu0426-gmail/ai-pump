...
- 正式 Query API 已成功返回、但本轮条件没有匹配到对象时，属于“已验证的负结果”，可以如实回答“未找到”；HTTP/协议失败、超时或未完成全部正式 API 调用属于“未验证失败”，不得与零结果混淆。
...
11. 自然语言理解由模型负责。每轮必须先提交结构化意图信封，至少包含当前目标、`conversation/query/analysis/command` 模式、业务域、上下文依赖、回答形式、歧义和最小能力步骤；不得用业务关键词正则、停用词删除或型号模式替代语义理解。正则只允许用于 ID、数值、单位、token 和传输协议等确定性语法校验。
...
13. 执行轮必须按模型生成的能力步骤顺序逐项开放当前能力；不得在规划后重新暴露同域全部工具让模型无约束改道。计划步骤完成后才可开放同域能力用于必要的补充查询。
...
15. 模型生成的每个工具参数都必须直接通过该 tool 的唯一 JSON schema。未知字段、错误类型、非法枚举、缺失必填和越界数值必须在调用正式 API 前拒绝；服务端不得再维护一份按自然语言删改参数的手写语义校验器。
16. 同一问题要求“明细和当前成本”等多个事实时，意图计划必须列出最小必要能力步骤，并在限定轮次和调用数量内编排对应正式 Query/Preview API；保存快照、知识快照和模型常识不得代替当前事实。
17. 正式 Query API 持有筛选语义；AI executor 只负责传递已校验参数和压缩展示结果，不得在 API 返回后另行筛选或重算。
18. AI 的历史消息窗口只能用于语言理解，不能作为可执行状态。上下文继承必须来自意图信封的结构化 `contextMode`：新的明确问题只保留当前轮；只有紧邻追问或缺参补充可保留上一轮；页面上下文只在明确指代时使用。跨业务切换不得继承更早的写意图、参数、确认或成功结论。
19. 模型返回的每个 tool call 必须同时属于服务端本轮实际下发的工具 allowlist，并符合当前轮次的读写意图；不满足任一条件时统一执行器必须在调用正式 API 或签发 confirmation token 前拒绝。历史写入指令、模型自行重放和并发旧请求均不能扩大本轮权限。
20. 查询结果必须返回已应用的结构化条件；零结果与读取失败必须是可区分的结果。计划中的必要能力未全部取得成功证据时，不得输出业务结论。
