...
MCP 写目录、确认协议、executor 或正式 command 变更还必须运行 `npm run verify:mcp-write-local`。该门禁以 `scripts/mcp-write-acceptance-manifest.cjs` 为显式验收清单，并与上述 17 个正式写工具做严格集合比对；快速协议矩阵逐工具覆盖 scope 拒绝、逐工具 allowlist 拒绝、Preview、HMAC 主体/工具/参数绑定、明确接受、确认层重放和拒绝无副作用。随后 `scripts/run-mcp-write-local-e2e.cjs` 创建全新临时 SQLite、随机 localhost 端口和 2025/2026 两代真实客户端，让 17 个工具逐一完成 MCP form elicitation → 正式 executor/API → operation/audit → Query/数据库回读闭环；2025 客户端还真实完成只读调用并拒绝隐藏写工具。四个代表性正式命令使用同一 `requestState/inputResponses` 做真实幂等重放，订单、库存、配方、文件、打印覆盖真实失败且零副作用；`update_recipe` 额外从空规格基线改为临时非空规格，再以 `clearSpec:true` 完成第二次原生确认和正式恢复，核对包含包装箱型与旧喷漆工资迁移状态的完整业务快照恢复、零件目录零副作用及第 18 条持久 operation。报价转订单和打印分别核对来源业务字段及完成任务的 `jobId/PDF`。报价、订单、配方、零件、线圈、文件和知识均只写临时库；FreeCAD、macOS `lp` 及 Windows Sumatra/Edge/rundll32 均由可计数替身拦截，未知外部命令 fail-closed，并回读异步终态。报告分别写入 `logs/mcp-write-local-latest.json` 和 `logs/mcp-write-local-e2e-latest.json`，明确记录 `productionTouched=false/physicalSideEffects=false`。本地 17/17 通过只证明代码链路具备受控写入条件，不构成生产写开关、身份或逐工具 allowlist 的授权。
