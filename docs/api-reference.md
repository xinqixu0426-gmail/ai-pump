...
当前源码共有 264 个 Express 路由声明。表内出现不代表推荐新调用：标为兼容或观察的入口仅供现有调用方迁移，新增页面、AI 工具和内部服务必须使用标准入口。
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `POST` | `/api/parts/:id/rename-impact` | `{ model, offset?, limit?, sourceHash? }` | 只读能力 `parts.rename_impact`，Web/Internal；返回拟改名称、引用数量/位置/状态、完整性、撞名及普通修改阻塞、当前资源版本和数据哈希。分页默认50、最大100，后续页必须携带 sourceHash，来源变化409；参数400、目标缺失/停用404。`displayOnly=true`，不写库、不签发改名或入库授权；无阻塞不等于完整规格核验通过。与改名保护复用检查，同一只读事务，no-store。Web 零件编辑页提供“检查引用”和分页加载；更换零件/拟改名称清空报告，15秒超时或翻页失败丢弃旧页后重查；未保存的供应商等字段不参与报告 |
...
零件列表及命令响应增加 `naming:null|{ruleId,ruleVersion,spec}`；既有零件保持 null，命名输入不证明完整实物规格。Web 零件页及模板/配方内“新增零件并选中”“集中补齐零件”新建上述四类时要求填写品名/明确规格/可选必要区别，自动预览型号，保存后回读。集中补齐在批量正式预览前逐行调用名称预览，按生成名检查重复及分类冲突，保留原草稿行标识用于回填；确认前可返回修改规格，修改后必须重新预览。已有生成名的普通 PATCH 和资料整单保存不允许更换型号、类别或命名规格；价格、库存和备注仍走原正式命令。旧零件提交 naming 返回409，等待后续受保护的规范化流程。未带 naming 的旧调用仍兼容原型号，不代表所有入口已强制命名；其他类别和 AI 自然语言命名录入尚未接入。

### 4.1 目录命名与引用

模板、配方和常用配置新建必须提供 `naming:{ruleId,spec}`，规则分别为template/recipe/model-variant；缺失命名返回400 CATALOG_NAMING_REQUIRED。服务端生成名称并在创建事务保存结构化身份档案，普通PATCH不能绕过改名。配方规则版本3支持 `spec.displayName`（1–180字、去首尾空白、禁止控制字符），名称原样采用，不附加“水泵”前缀或工程参数；旧 `series/configuration` 输入继续支持，生成名也不再添加“水泵”前缀。定子、片数及已知机筒长度取保存参数；重复提供且不一致返回422 CATALOG_NAMING_SPEC_MISMATCH。displayName仅为名称，不参与实物规格指纹。新建externalModel可独立提供，缺省保留原name或生成名，草稿hash绑定命名信息与对外型号。Web先选模板，自动带入去掉模板/泵壳/水泵标识的名称，允许编辑，不重复填写系列与配置区别；已有名称不会批量重写，存量规范配方改名仍走正式rename-preview/rename协议。整套模板必须提供具体shellPartId，类别/停用校验后保存独立绑定，多供应商不能任取。AI create_recipe继续要求naming、coilSpec、coilSheets及正式线圈ID或方案系列，通过正式保存草稿/API建档。

泵壳默认轴承写入保存defaultUpperBearingPartId/defaultLowerBearingPartId；输入旧代号只在活动目录唯一匹配时补ID，多供应商需明确选择。长螺丝定价保存screwPricingPartId，后续按该ID校验来源活动状态、直径和供应商，无效ID不回退其他来源或公式。没有具体目录来源的纯公式计价仍不虚构ID。
...
| `POST` | `/api/catalog/migration-preview` | `{ entries: [{ entityType, entityId, naming: {ruleId,spec}, samePhysicalItem:true, expectedUpdatedAt }] }`，1–100条、严格字段 | `catalog.migrate`只读批量预览；复用正式改名校验，拒绝重复对象和批内生成名冲突。冻结完整原目录/引用来源，返回entries、confirmationToken和suggestedIdempotencyKey；不写库 |
| `POST` | `/api/catalog/migrate` | `{confirmationToken,idempotencyKey}`，严格字段 | `catalog.migrate`，high，web/internal；同主体凭证与持久幂等。先全批复验冻结来源，再在同一SQLite事务逐条调用正式改名服务并续接相交引用；任何校验、强审计或写入失败整批回滚。返回标准CommandReceipt含migratedCount、entries、changes及auditIds。只接受人工核对的明确规格，不猜测、合并、删除未知对象；原JSON、数量、价格、库存和进度不改。400输入、404失效、409并发/冲突；恢复使用正式数据库备份/恢复服务 |
...
| `DELETE` | `/api/coils/:id` | `{ expectedUpdatedAt?, idempotencyKey? }`；推荐请求头 `Idempotency-Key` | 能力 `coils.delete`。仅允许删除库存为 0 且从未产生库存流水的线圈方案；已有规范身份档案时改为停用并保留档案和绑定，不做硬删除；已有库存或流水时返回 `409`，避免破坏库存追溯。页面继续显式确认；旧空请求兼容执行并返回缺少版本/重试保护 warning |
...
| `POST` | `/api/templates` | 请求头 `Idempotency-Key`；必须提供 `naming:{ruleId:"template",spec:{series,configuration}}`；名称由服务端生成，`bundle` 必须提供具体 `shellPartId`；其余模板字段；`bundleNote` 为泵壳套件备注；`shellComponentsJson` 在 `components` 模式下保存自由组合计价项，只接收 `partId/name/model/supplier/qty/unitCost/pricingMode/included/optional/componentType/subassemblyContents/note`，未知字段明确忽略；历史 `isStainlessStretchBarrel` 仅用于只读识别，不保存。每个计入项的 `qty` 必须为正数，`model` 必须存在于零件库“泵壳搭配”分类，否则返回 400。普通单件使用 `componentType=standard`；不锈钢拉伸筒使用 `stainlessStretchBarrel`；供应商小套件使用 `subassembly` 并带一级 `subassemblyContents: [{ name, qty, referenceUnitPrice?, note? }]`，其中 `referenceUnitPrice` 可省略、提供时必须为非负数。计入的小套件至少 1 个、最多 30 个组成项，组成项数量必须为正数。自由搭配最多 50 个计价项。`surfaceTreatmentMode` 支持 `none/painting/electrophoresis/electrophoresis_powder_coating/powder_coating`，`surfaceTreatmentCost` 为非负费用 | 能力 `templates.create`。新增模板并返回原模板顶层字段和标准 operation receipt；模板、operation 与强审计同一事务。`bundle` 模式通过 `shellPartId` 独立绑定活动泵壳零件，`shellModel` 保存系统生成的模板名称；`components` 模式的单件或小套件父项必须引用“泵壳搭配”零件。Web 选择父型号时将该型号当前目录价带入 `unitCost` 作为备用单价；正式计价仍优先读取当前零件库价格。小套件父项是唯一计价和库存单位，组成项的 `referenceUnitPrice` 只用于查询、小计和套件差额比较，不生成独立 BOM、不重复计价或扣库存；历史组件名称和缺少参考单价的组成项继续兼容读取 |
| `PATCH` | `/api/templates/:id` | 请求头 `Idempotency-Key`；已有模板业务字段及 `expectedUpdatedAt`；规范名称修改走 `catalog.rename`，普通编辑不要求新增用的 `naming` | 能力 `templates.update`。资源版本、组件目录校验、模板更新、operation 与强审计同一事务；原 URL 和模板字段保持兼容 |
| `DELETE` | `/api/templates/:id` | 请求头 `Idempotency-Key`；请求体或查询参数 `expectedUpdatedAt` | 能力 `templates.delete`。无任何历史配方引用时软删除并保留身份档案、绑定和审计，返回标准回执；有引用返回 `409 template_in_use`。原删除保护语义保持不变 |
...
模板成本、默认配方和应用模板的只读查询在同一SQLite事务读取模板和目录；保存了 `partId` 的固定件按 ID 装配现名，成本查询的组件同时按该 ID 选价并将 ID 传给成本引擎。默认/应用结果的 `template` 保留原快照，`parts/recipeDraft.partsJson` 使用现名与原 ID，不回写模板或签发改名/入库授权。失效、停用、供应商冲突或组件错分类返回422，不回退同名物料；物料JSON损坏、非数组或超过1000项也返回422。未带 ID 的旧成本与备用价规则保持兼容，原列表/详情仍保留保存内容。

模板固定配件 `partsJson[]` 与自由搭配 `shellComponentsJson[]` 可保存可选 `partId`。Web 编辑、复用及单条/批量建档回填保留该 ID；手动更换型号或供应商后按唯一目录记录重绑，无法唯一确定则清空旧 ID。正式新增/修改事务校验所有显式 ID（包括未计入组件）的有效性、现型号和明确供应商，组件 ID 还必须属于泵壳搭配；无效、停用或冲突返回 422，整单不写入。未带 ID 的旧模板仍兼容，不在编辑中猜补历史身份；整套泵壳的绑定迁移仍另行处理。
...
| `POST` | `/api/model-variants` | `templateId` 和 `naming:{ruleId:"model-variant",spec:{series,configuration}}` 必填，`modelName` 由服务端生成；可带线圈、机筒、长螺丝、叶轮字段、`customFieldsJson` 和 `idempotencyKey?`；精确线圈片数用 `coilId`，非精确片数用 `coilSchemeFamilyCode`；推荐请求头 `Idempotency-Key` | 能力 `model_variants.create`。新增常用配置预设；线圈选择按精确方案或插值/外推系列持久化。若模板含长螺丝且预设有机筒长度，会按参数化螺丝公式自动补齐对应长度的螺丝零件。常用配置预设、自动生成零件、operation 和全部强审计同一事务提交；历史顶层字段及响应顶层 `createdLongScrewParts` 保持兼容 |
| `PATCH` | `/api/model-variants/:id` | 已有配置业务字段（新增必填 `naming` 不适用于普通编辑），另带 `expectedUpdatedAt?`, `idempotencyKey?`；推荐请求头 `Idempotency-Key` | 能力 `model_variants.update`。更新常用配置预设并校验、持久化精确 `coilId` 或非精确 `coilSchemeFamilyCode`，同时可能沉淀新的长螺丝规格；新调用绑定资源版本，旧无版本请求兼容执行并返回 warning；历史兼容字段与 `createdLongScrewParts` 保持兼容 |
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `GET` | `/api/recipes` | 可选 query：`keyword`、`hasTechnicalFiles=true/false` | 经纯读 `recipeQueries` 返回配方列表；默认列表和筛选结果均返回 `technicalFileCount`，`keyword` 对成品型号（字段 `name`）和配置摘要（字段 `spec`）做模糊筛选，`hasTechnicalFiles` 按有效技术档案关联筛选。标准字段含 `id/createdAt/updatedAt/externalModel/customBarrelLength/longScrewExtraLength/configurationPolicyJson`；原始保存内容另提供 `snapshotPartsJson/snapshotExtraPartsJson/snapshotPackingPartsJson/snapshotConfigurationPolicyJson`，现名投影与原快照分开 |
| `GET` | `/api/recipes/:id` | 无 | 经纯读 `recipeQueries` 返回单个配方，标准字段含 `id/createdAt/updatedAt/externalModel/customBarrelLength/longScrewExtraLength/configurationPolicyJson`；原始保存内容另提供 `snapshotPartsJson/snapshotExtraPartsJson/snapshotPackingPartsJson/snapshotConfigurationPolicyJson`，现名投影与原快照分开 |
...
| `DELETE` | `/api/recipes/:id` | 请求头 `Idempotency-Key`；请求体或 query `{ expectedUpdatedAt? }`，也兼容 `If-Unmodified-Since` | 能力 `recipes.delete`。配方软删除、规则学习刷新、operation 和强审计同一事务提交；相同请求安全重放，版本、异参复用或审计冲突返回 409。Web/AI 传递当前资源版本；旧请求缺少重试/版本协议字段仍兼容并返回 warnings；新建缺少结构化 `naming` 返回 `400 CATALOG_NAMING_REQUIRED`，不能绕过生成规则 |
...
| `POST` | `/api/recipes/:id/scenario-compare-preview` | `{ version:1, baselinePolicy:"CURRENT_REBUILT", scenarios:[{ scenarioKey, label, overrides }] }`；`overrides` 支持浮球、电缆、正式线圈、机筒、`packingParts`、`surfaceTreatmentMode`、`surfaceTreatmentCost` | 正式能力 `recipes.scenario_compare_preview`（preview，低风险，无确认、审计或业务写入）。在单一同步只读事务中读取配方及本次 BOM、线圈和设置实际使用的正式记录，并以 `costEngine + recipeBomEngine` 对 `base` 与 1–3 个候选配置重建。`packingParts` 每项必须提供正式 `partId/model/supplier/qty/packingRole`，服务端重读活动“包装”目录记录、校验身份与正式包装角色；非 fixed 角色不可重复，补丁只替换其角色并保留无关 fixed 包材。客户端价格、成本和铜价被拒绝。表面处理只接受已登记 mode；`none` 强制费用为 0，改变 mode 时只可使用用户明确费用或当前 `configurationPolicyJson.surfaceTreatmentOptions` 的正式 mode/cost 对，不能继承旧 mode 费用。响应的 `recipe` 是 CanonicalEntity；`changes[]` 每项为 `{scenarioKey,field,from,to}`。响应还包含同一读取集合的 `readSetId/readSetHash`、有界 `sourceVersions`、实际 requested/applied/notApplied、配置 hash 与同口径 delta。未启用浮球或电缆时，其依赖字段进入带原因码的 `notApplied`，不得静默生效。缺价时总额及差额为 `null`；不创建临时配方、operation、审计或文件。 |
| `POST` | `/api/cost/profitability-preview` | `{ version:1, basisRef:{kind:"SCENARIO_COMPARISON",recipeId,comparisonInput,scenarioKey}, unitPrice, quantity:null|positiveInteger, currency:"CNY" }` | 正式能力 `cost.profitability_preview`（preview，低风险，无确认、审计、operation 或业务写入）。服务端每次重新执行 `recipes.scenario_compare_preview`，只取同一 read set 中指定 `base` 或候选情景的完整 `currentTotalCost` 作为 `unitCost`，再由共享 `profitabilityPreview` 服务计算单台毛利、销售毛利率和成本加价率。响应返回该正式情景的 `configurationHash`，用于 Task V2 验证与其他 Preview 的同一配置，不要求成本与库存 read set 相同。只支持 CNY；`quantity:null` 不默认 1；成本不完整时成本、毛利和比例均为 null，不能用 partial cost。结果明确为毛利试算，不包含运费、税费、汇率或财务费用；旧报价/订单的 `margin` 价格系数不参与本接口。 |
| `POST` | `/api/inventory/virtual-readiness-preview` | `{ version:1, basisRef:{kind:"RECIPE_SCENARIO",recipeId,comparisonInput:{version:1,baselinePolicy:"CURRENT_REBUILT",scenarios:[]},scenarioKey:"base"}, quantity:positiveInteger<=100000 }` | 正式能力 `inventory.virtual_readiness_preview`（preview，低风险，无确认、审计、operation、订单、采购或库存写入）。只接受已绑定配方 ID 和正式情景比较请求，`scenarioKey` 必须为本次结果中的 `base` 或指定候选；拒绝客户端库存、短缺、预留和“忽略订单”参数。服务端在单一同步只读事务内以 `orderPlanning` 与活动订单同一 identity、库存单位、`stockQtyPerUnit`、占用分配和短缺投影计算虚拟数量需求，固定口径为 `CURRENT_STOCK_AFTER_ACTIVE_ORDER_RESERVATIONS`。响应返回 `READY|SHORTAGE|INCOMPLETE`、`configurationHash`、完整 `readSetHash`、有界 `sourceVersions`、requirements/shortages/unresolved/excluded 与 coverage；只有全部库存管理需求已评估时才返回 READY 或 SHORTAGE。缺价不阻止库存齐料；未绑定或无法验证物料身份、情景失败或超过安全结果上限返回 INCOMPLETE，不能称为仅缺某几项。此 Preview 不创建临时订单或预留，重复调用同一快照结果一致。 |
...
`configurationPolicyJson` 版本 1 结构为 `{ version: 1, fields?, packingPartIds?, packingRemovalPolicy?, surfaceTreatmentOptions? }`。`fields` 可包含 `hasFloat/floatWire/floatAccessoryType/hasCable/cableLength/cableWire/cableAccessoryType/coilSpec/coilSheets/coilMaterial/coilSlotType/customBarrelLength` 的允许值数组；`packingPartIds` 是控制新增或同角色替换的稳定正整数包装零件 ID 数组。`packingRemovalPolicy` 可选且显式开启严格删除控制：`{ removableRoles: (container|pearlCotton|foam|fixed)[], removablePartIds: positiveInteger[], allowClearAll: boolean }`；`qty=0` 的明确删除须由 role 或精确 part ID 允许，`packingParts=[]` 只由 `allowClearAll:true` 允许，同角色替换不属于删除。该字段缺失时保留历史 `legacy_open` 语义。`surfaceTreatmentOptions` 是 `{ mode, cost }[]`：字段存在时，任何表面处理覆盖必须精确匹配一个正式 mode + cost pair；mode 不存在或费用不同均以 `RECIPE_CONFIGURATION_SURFACE_NOT_ALLOWED` 拒绝。字段不存在时，read-only 情景可使用用户明确提供的非 `none` mode + cost 作为 hypothetical 输入；仅给出 mode 时返回 `SURFACE_TREATMENT_COST_REQUIRED`。未知键、未知字段、重复工艺或无效类型返回稳定的 `RECIPE_CONFIGURATION_POLICY_*` 400 错误；单个列表最多 100 项。
...
| `GET` | `/api/copper-price` | 无 | 只读查询每日铜价快照和数据库已采用基数；返回 `source/sourceOfTruth/asOf/stale/lastError`。不访问外部行情、不写库；无成功快照时立即返回 `503 market_snapshot_unavailable` |
| `POST` | `/api/copper-price/update` | 推荐请求头 `Idempotency-Key` | 维护能力 `market.sync_copper_price`。使用当天共享行情快照；当天未尝试时在事务外获取一次，再只更新 `calculated` 模式中基数或成本发生变化的线圈；`kit` 不变。过期行情拒绝改价。线圈、operation 和逐项强审计同一事务，返回 `updatedCount/skippedCount/unchanged` 及标准回执 |
| `GET` | `/api/market-indicators` | 无 | 只读读取每日铜/铝/美元汇率快照及数据库已采用值；返回 `sources/sourceOfTruth/asOf/fetchedAt/stale/lastAttemptAt/lastError`，不访问外部行情、不写库。旧快照保留并标明 `stale=true`；无快照时立即返回 `503 market_snapshot_unavailable` |
| `POST` | `/api/market-indicators/update` | 推荐请求头 `Idempotency-Key` | 维护能力 `market.sync_indicators`。共享当天快照，不重复抓取；仅当天快照允许同步。只更新 `calculated` 铜价/成本；`kit` 不变。与铝线价格基数、美元汇率、operation 和强审计在同一 SQLite 事务提交；同键重放首次回执 |
...
两项市场同步都是显式 maintenance，不开放 AI 写工具。每天 15:00 BJT 调度，启动时补齐当天尚未尝试的获取；同一北京日期最多一轮获取（每个行情源有超时及一次有限重试）。`dailyMarketSnapshot` 在 `system_settings.market_snapshot_daily_cache` 保存快照和 `runId/startedAt/completedAt/status/changedCount/lastAttemptDate/lastError`；事务内先记录当日尝试，并用单飞合并并发请求，重启或失败后不在当天重复获取。失败保留上次成功快照，旧快照只供带时间标记的展示，不允许自动或手动改价。缓存维护是可重建技术过程，与正式成本同步的强审计/operation 事务分离。GET 只读，不触发补抓；页面线圈列表与行情独立加载、独立显示失败。同步不提供 Preview/确认弹窗：网页按钮/定时任务本身是明确动作；替代保护为当天快照、外部超时/有限重试、`502 market_data_unavailable` 或 `503 market_snapshot_unavailable`、90 天持久幂等、原子提交、逐项强审计和标准回执。同键重放返回首次已提交结果。
...
| `POST` | `/api/orders` | 请求头 `Idempotency-Key`；请求体为 `/save-payload-draft` 返回 payload（包含规范化 `naming/externalModel`），并携带 `previewHash?` | 能力 `orders.create`；新增订单固定进入“待确认”。订单、operation 回执和强审计同一事务提交；相同请求安全重放，预览篡改、异参复用或审计缺失整体回滚。为保持现有页面兼容，响应顶层继续提供完整订单字段，同时增加 `order`、`operationStatus` 和标准回执字段；旧请求不传协议字段仍兼容并返回 warnings |
...
单项进度、旧下单兼容入口和跨订单批量下单都要求目标配置唯一。未传 `identityKey` 时，同名不同配置返回 `PURCHASE_TARGET_AMBIGUOUS`（409）；传入过期键不能回退名称。已有明确零件/线圈 ID 的无效、缺失或停用目标不回退同名物料；ID＋型号不一致仍拒绝，历史显示绑定不能绕过实物身份校验。采购计划规范化前校验库存类型与零件/线圈 ID，冲突返回 `PURCHASE_INVENTORY_TYPE_MISMATCH`（409），不能通过覆盖类型消除冲突；明确声明的目录供应商不一致返回 `PURCHASE_PART_SUPPLIER_CHANGED`（409），不能用目录现值覆盖旧 BOM 中的冲突。`actualSupplier` 保留为实际供货事实，不参与库存目标定位。未声明供应商的旧引用仍沿用严格 ID＋型号检查。入库预览和执行共用目标、目录供应商、数量及换算验证（入库层兼容包装错误为 `purchase_inventory_mapping_invalid`/400），线圈只接受正式方案和整数数量，电缆支持小于1米的正换算率；零增量不写库存。多条成品电缆保留各自配置，含义不明的新旧电缆混合返回 `CABLE_LEGACY_CONFIGURATION_AMBIGUOUS`，不再只保留第一条。
...
| `POST` | `/api/rotor/recipe-draft` | `{ recipeId }` | 正式能力 `rotor.recipe_draft`（query，来源 `recipes+pump_shell_templates+unique_active_shell_catalog+rotorTemplateDraft`，Web/internal，low，15秒，无确认/幂等/审计）；根据配方技术档案生成出图表单草稿；配方录入的转子出图参数优先于泵壳模板历史默认值，并带入成品型号（字段 `name`）、机筒长度和不锈钢机筒开档；模板泵壳目录多候选时与 template-draft 相同，返回 `409 PUMP_SHELL_PART_AMBIGUOUS` 及候选，不能任取一条带入参数；不写库 |
| `POST` | `/api/rotor/template-draft` | `{ templateId, variantId? }` | 正式能力 `rotor.template_draft`（query，来源 `pump_shell_templates+unique_active_shell_catalog+rotorTemplateDraft`，Web/internal，low，15秒，无确认/幂等/审计）；根据泵壳模板和可选常用配置预设生成出图表单草稿，带入轴承、油封、泵壳 notes 默认参数、不锈钢机筒开档和图纸备注；`variantId` 为历史兼容字段；泵壳精确匹配和历史尺寸后缀兼容匹配都要求唯一活动目录记录，同名多供应商返回 `409 PUMP_SHELL_PART_AMBIGUOUS` 及 `details.candidates[{partId,model,supplier}]`，不自动选第一条参数；不写库 |
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `POST` | `/api/relations/read` | `{version:1, relation, rootId?, pageSize?, afterId?, stockStatus?}` | 能力 `relations.read`；严格类型化字段，调用方只能提交 relation/root/分页，SQL 由服务端决定。已挂载在 `api.cjs` 认证后的 `/api` 段（`x-internal-secret` 内部调用放行）。关系：`customer.orders`/`order.customer`/`order.lines`/`recipe.parts`/`part.recipes`/`coil.recipes`/`parts.stock`/`part.facts`；其中 `coil.recipes`（ONT-P8R）按 `recipes.coil_id` 外键有界反查引用某线圈的配方，语义 `CURRENT_RECIPE_COIL_REFERENCES`。返回 BoundedRelationResultV1，不返回原始订单/BOM JSON。AI 侧由 `get_recipes_by_coil` 使用 `coil.recipes` |
| `POST` | `/api/relations/resolve` | `OntologyRelationRequestV1`：`{ontologyVersion:1,relationId,root:{entityType,canonicalId},pageSize?,afterId?}` | 能力 `ontology.relations.resolve`；复用正式 Ontology Resolver，只接受 canonical root。返回标准 envelope，成功数据为 `OntologyRelationResultV1`；未知关系/根为 404，旧引用歧义为 409，非法请求为 422，技术失败为 500。私有 AI 的 `get_recipe_parts/get_recipes_by_part` 通过本入口读取 `recipe.contains_part/part.contained_in_recipe`，单次工具调用最多 8 页且累计小于 32 KB |

`/read` relation 仅支持 customer.orders、order.customer、order.lines、recipe.parts、part.recipes、coil.recipes、parts.stock、part.facts。除 parts.stock 外要求规范 rootId，调用方须先通过正式查询确认目标。页默认20、最大50，id DESC + keyset，正式 COUNT/精确关系计数，hasMore/pageBoundary，逐页只读事务，不承诺跨页历史快照。单客户/两事实请求禁止 afterId。完整结果仍受262144字节上限保护，不提高现有全局上限。
...
| `POST` | `/api/ai/chat` | `{ messages, conversationId?, providerPreference?, pageContext?, resolutionContext?, turnState? }` | SSE 流式对话；`providerPreference` 只接受 `default/local/deepseek/kimi`，手工选择仅影响本轮且不跨模型降级；消息可带 `attachments: [{ id }]`；`conversationId=chat-<ID>` 可按当前所有者恢复持久化的可信候选；`pageContext` 当前仅接受白名单化的订单 `resourceType/resourceId/view`；`resolutionContext` 与 `turnState` 仅保留兼容传输，不作为候选身份、执行证据或授权。AI Native rollout 仅由服务端环境快照选择：生产为 `AI_NATIVE_MODE=owner`，仅接受现有正式 Owner JWT，普通 admin、内部请求和自报 header 不可进入（403 `AI_OWNER_ONLY`）。`off`/`shadow` 现在只表示 AI 不可用（403 `AI_UNAVAILABLE`），**不再回退任何 Legacy 运行时**；退役运行时已由 NATIVE-HC2 删除。NATIVE-W1.5：写/命令意图的走向由**服务端** rollout 快照决定，请求体不能影响该判定。`AI_NATIVE_WRITE_ENABLED=false`（生产默认）时仍是 `status.stage=native_write_disabled` 与 `detail.state=WRITE_DISABLED`（含稳定的 `commandLabel` 能力描述，不再输出内部路由对象），零任务、零预览、零 token、零写入。写开关打开时只对 W1 唯一白名单能力 `adjust_part_stock` 生效：确定性解析「库存 + 明确增减 + 明确数量」（不要求用户说出「零件」，0 次额外模型调用），目标身份交给既有 canonical resolver（0 匹配 / 多匹配分别给出安全结论），随后**同步**走既有合法状态序列并调用既有 W1 正式预览，把任务推进到 `WAITING_APPROVAL`，再下发结构化事件 `{ type: "write_proposal", stage: "NATIVE_WRITE_PROPOSAL", proposal:{kind,capabilityId,items:[{partId,model,currentStock,delta,nextStock,clampedToZero}]}, confirmation:{confirmationToken,operationId,expiresAt,toolName,args}, task:{taskId,revision,state,statusPath} }`；该轮**绝不执行写入**，执行仍需 Owner 对同一冻结提案调用 `write-execute`。**V1 只支持 delta（按数量增减）**：绝对目标值语法（如「库存增加到 30」「库存减到 20」「库存改为 30」「库存设为 30」「库存到 30」）先于增量抽取被识别，一律返回 `detail.state=WRITE_CLARIFICATION_REQUIRED` + `code=NATIVE_WRITE_ABSOLUTE_STOCK_UNSUPPORTED`（「当前只支持按数量增加或减少库存」），**绝不把最终值当成增量**。不完整/歧义意图同样返回 `WRITE_CLARIFICATION_REQUIRED`（如 `NATIVE_WRITE_QUANTITY_REQUIRED`、`NATIVE_WRITE_QUANTITY_INVALID`（含 0）、`NATIVE_WRITE_QUANTITY_AMBIGUOUS`（如「增加 100 还是 200」）、`NATIVE_WRITE_ACTION_AMBIGUOUS`（方向不明或同句增减冲突/负号）、`NATIVE_WRITE_TARGET_REQUIRED`、`NATIVE_WRITE_SINGLE_TARGET_REQUIRED`、`NATIVE_WRITE_SOURCE_MISMATCH`），白名单外写意图返回 `detail.state=WRITE_UNSUPPORTED`，以上都不生成提案、不建可写任务、不产生任何业务写入。 |
...
| `GET` | `/api/ai/tasks/:taskId/events?afterSeq=0&limit=50` | `afterSeq>=0`，`limit=1..100` | 返回增量 `TaskEventsPageV1`；事件为脱敏状态投影，GET 不会续租、改状态或调用业务能力。 |
| `POST` | `/api/ai/tasks/:taskId/resume` | `TaskResumeRequestV1`；必须请求头 `Idempotency-Key` | 显式后台继续。`expectedRevision` 与当前任务一致、choice 属于当前候选集合且未过期、补充内容已作为同 owner/同会话的 user 消息持久化并被服务端精确重绑时才接受；更新计划后返回 `202 TaskAcknowledgementV1`。终态任务不会原地恢复。 |
| `POST` | `/api/ai/tasks/:taskId/cancel` | `TaskCancelRequestV1`；必须请求头 `Idempotency-Key` | 停止未来执行。无租约且没有已准入命令时立即 `CANCELLED`；已运行或存在 `COMMAND` step 的任务先持久化取消请求。可能存在或已经确认的写效果进入 `RECONCILING`，不声称已回滚。 |
| `POST` | `/api/ai/tasks/:taskId/write-preview` | `TaskWritePreviewRequestV1` | N6.1 引入的受保护写预览在 NATIVE-W1 收敛为：只为唯一获批写能力 `adjust_part_stock`（capability `inventory.parts.batch_adjust_stock`）生成正式预检确认卡；任何其它写能力（含 `batch_update_prices`、删除类、多目标批量）一律拒绝，不静默收窄或拆分。作用域受 `api/services/aiNativeWriteScope.cjs` 的默认拒绝白名单约束：非白名单能力 `403 NATIVE_WRITE_CAPABILITY_UNSUPPORTED`，一次请求出现多个不同目标 `409 NATIVE_WRITE_SINGLE_TARGET_REQUIRED`，缺少目标型号 `400 NATIVE_WRITE_TARGET_MODEL_REQUIRED`，调整量为零或非整数 `400 NATIVE_WRITE_QUANTITY_INVALID`。任务必须为 owner-bound `DETACHED`、`CONFIRMATION_REQUIRED`，且目标为 `PREPARE_CHANGE`/`APPLY_CHANGE`。服务端将 `{ taskId, planRevision, goalKey }` 绑定到 token 的私有 executionContext；卡片不执行写入，token、预检上下文、lease、参数和写权限不进入公开任务视图。响应除确认卡外新增冻结 `proposal`（`partId`/`model`/`currentStock`/`delta`/`nextStock`/`expectedUpdatedAt`/`clampedToZero`）与稳定 `idempotencyKey`，同一提案同时持久化为 `spec.writeV1`（phase `PROPOSAL_READY`）并自转移 `WAITING_APPROVAL`；存在 `RUNNING`/`UNKNOWN_EFFECT` 的 COMMAND step 时禁止重新预览（`409 TASK_WRITE_INFLIGHT`），防止覆盖在途写入。过期、错主体、版本/参数漂移、旧卡或进程重启均须重新预检。审批人必须是**规范 Owner 身份**：`x-internal-secret` 等内部机器凭据、普通 admin JWT 或未认证一律 `403 NATIVE_WRITE_OWNER_REQUIRED`（内部凭据只授权服务到服务执行，不构成用户批准）。AI Native rollout 还要求当前请求启动快照同时为 `AI_NATIVE_MODE=owner`、正式 Owner JWT 和 `AI_NATIVE_WRITE_ENABLED=true`；默认 `false`，请求体、模型内容或 header 不能改变该开关。 |
| `POST` | `/api/ai/tasks/:taskId/write-execute` | `TaskWriteExecuteRequestV1` | N6.2 的执行语义在 NATIVE-W1 收敛为：只执行同一 owner-bound task 已预检的 `adjust_part_stock` 确认卡（其余写能力在作用域层即被拒绝）。服务端在正式命令发送**之前**先把 Owner 批准事实（`proposalHash`/`argsHash`/`capabilityId`/`toolName`/`targetPartId`/`ownerSubject`(sha256)/`approvedAt`/`idempotencyKey`）持久化为 `spec.writeV1.phase=CONFIRMED` 并追加 `WRITE_APPROVED` 事件，再持久化 COMMAND step 与预检幂等键；实际写入仅经 `executeConfirmedAiTool` 与原正式业务 API，重复提交复用同一幂等键由 `api_operations` 唯一约束去重，不会二次调整。冻结提案与当前预检不一致时拒绝执行。写入后必须完成独立回读核验（目录重新读取 + 冻结 `nextStock` 逐项比对）才置 `VERIFIED`/`SUCCEEDED`，回读缺失或不一致即安全失败，绝不宣称成功。响应丢失不会生成新 operationId 或重放命令，而是进入 `RECONCILING`。响应只返回脱敏正式 operation/audit 摘要，并新增 `outcome`（`phase`/`verified`/`stock`/`operationId`/失败安全标志）。除 N6.2 原有保护外，Native rollout 必须同时通过 `AI_NATIVE_MODE=owner`、正式 Owner JWT 和独立的 `AI_NATIVE_WRITE_ENABLED=true`；`owner` 模式本身绝不启用写入。 |
| `POST` | `/api/ai/tasks/:taskId/write-reconcile` | 无 | 仅对 owner-bound `RECONCILING` task 的唯一 COMMAND step 进行当前主体受限的只读正式 operation 查询；与 `write-execute` 受**同一个** Native 写 rollout 授权与 Owner 身份门约束，任一不通过即拒绝。已持久化且可信的 COMPLETED operation 仍必须经独立回读核验 `nextStock` 才能置 `SUCCEEDED`/`VERIFIED`；MISSING 直接安全失败（`OPERATION_MISSING`，`reconfirmRequired`）；AMBIGUOUS 安全失败并转人工复核（`OPERATION_AMBIGUOUS`）；PENDING 只做**有界**等待（最多 5 次、10 分钟），超界即安全失败（`RECONCILIATION_BOUND_EXCEEDED`），任务绝不停留在 `RECONCILING`。任何情况下都不得换幂等键重试、不得自动重发写入、不得推断成功。响应新增 `outcome`。 |
...
MCP 变更在发布到 Mac Mini 前运行 `npm run verify:mcp-local`：它组合协议/安全单测、官方 conformance 场景，以及基于临时数据库和隔离 API 的真实 HTTP 验收；Hermes 兼容的 2025 客户端与通用 2026 客户端分别发现并调用全部 48 个只读工具，2026 写身份再完成一次 `sync_factory_knowledge` 正式 Preview → form elicitation → Command → operation/audit 回执闭环。隔离进程仅为连续调用把测试限流提高到 600，所有写入只发生在临时数据库副本，生产默认值和源数据库不变。正式环境部署后的日常发布原本自动运行 `npm run verify:mcp-prod-read`（严格核对 48 个只读工具及 annotations；若验收身份处于写灰度，只允许额外出现其逐工具 allowlist 中的写工具，未知或越权目录会失败。随后在一个连接内复用三个成本场景，并覆盖库存、配方、模板详情、客户/报价详情、订单/采购、管理/质量、知识、出图历史和统一业务变更；每个结果必须带已验证执行证据、能力 ID 和正式数据源，并核对完整资源没有大小写重复键或业务字段裁剪。生产空数据不视为协议失败，也不为覆盖详情造数据；全部 48 个工具和缺价路径仍在隔离库验收），但 **2026-09-19 起该步骤在发布流程中默认停用**：生产只有 1 个未删除配方，其成本对比场景要求至少两个正式配方，会阻断任何发布；负责人同时表示若 ontology 能满足需求可能弃用 MCP。恢复方式：`npm run deploy:macmini -- -McpAcceptance enabled`。服务端 `/mcp` 路由、只读工具目录、隔离库验收与手工执行该命令均不受影响。生产写开关的发现和单笔可回滚验收仍必须另行明确批准。详细流程和 Windows conformance CLI 的退出兼容边界见 `docs/mcp-development-guide.md`。
...
| `recipes.by_coil` | AI/内部 `get_recipes_by_coil`（`POST /api/relations/read`，`relation=coil.recipes`） | query/read | `recipes.coil_id` 当前规范引用 | low | 否 | 不适用 | 不适用 | 严格只读、有界键集分页 | 非 HTTP 默认面，仅经关联读取契约 |
| `ontology.relations.resolve` | HTTP/私有 AI `get_recipe_parts`、`get_recipes_by_part` | query/read | Ontology relation registry + canonical business tables | low | 否 | 不适用 | 天然幂等 | canonical-only、有界分页、严格只读 | 默认 HTTP 超时；AI 聚合最多 8 页/32 KB |
...
| `coils.delete` | HTTP/Web | command/write | 线圈库存 + 流水 | high | 页面删除确认 | 无 | 90 天持久化幂等 + `expectedUpdatedAt` | 追溯保护、删除/规范档案停用、operation 和强审计同一 SQLite 事务 | 默认 HTTP |
...
| `templates.delete` | HTTP/Web | command/write | 模板 + 全部历史配方引用 | high | 页面删除确认 | 无 | 90 天持久化幂等 + `expectedUpdatedAt` | 引用检查、软删除、operation、强审计同一 SQLite 事务 | 默认 HTTP |
...
| `market.sync_copper_price` | HTTP/Web/Internal scheduler | maintenance/write | 每日行情快照 + `coils.copper_base/cost` | high | 页面按钮、启动补跑或定时调度本身是明确触发 | 不适用；使用当天快照 | 90 天持久化幂等；当日获取尝试持久化 + 单飞 | 缓存维护独立；变化线圈、operation 和逐项强审计原子提交 | 25s |
| `market.sync_indicators` | HTTP/Web | maintenance/write | 每日行情快照 + `coils` + `system_settings` | high | 页面同步按钮是明确动作 | 不适用；使用当天快照 | 90 天持久化幂等；同键重放首次快照；当天不重复抓取 | 缓存维护独立；线圈、两个设置、operation 和全部强审计原子提交 | 25s |
...
| `DELETE` | `/api/ai/conversations/:id` | `{ expectedUpdatedAt?, idempotencyKey? }`；推荐请求头 `Idempotency-Key` | 能力 `ai.conversations.delete`。软删除会话；历史消息保留在数据库中但不再展示。存在非终态 AI Native 后台任务时返回 409，须先停止或等待任务完成，避免删除其恢复来源。新调用绑定会话版本并持久幂等，旧空请求兼容 |
...
- `search_coils`：调用 `/api/coils` 读取线圈/定子方案的实时完整档案，按 `spec/sheets/material/slotType/schemeCode/schemeStatus/isDefault/ratedVoltageV/ratedFrequencyHz/market/schemeFamilyCode` 类型化筛选；除库存、成本和方案状态外，还返回稳定方案编码、默认标记、电压、频率、市场、方案族、默认电容、默认搭配电缆横截面积及主副线漆包线线径/绕组数据。测试或停用方案继续携带明确 `schemeStatus`，空字段表示该方案未填写，不能推断系统没有该字段。不用知识快照回答当前线圈档案、库存或成本。按 `material`/`slotType` 收窄且同一 `spec-sheets` 还有其它正式方案时，响应额外带 `sameSpecSheetsVariants` 与 `sameSpecSheetsNotice`，明确本次结果是收窄后的子集、不是该规格片数的全部方案；`calculate_coil_cost` 按材质、槽眼或 `coilId` 收窄计算时同样附加这两个字段（同一个「规格-片数」可能有多套正式方案，例如 12-220 = 钢带/小眼 + 冷轧/国标眼，两套都可能是各自组合下的默认）。
- `get_recipes_by_coil`：ONT-P8R 有界规范反查。按一个已确认的正式线圈方案 ID 调用 `POST /api/relations/read`（`relation=coil.recipes`），从 `recipes.coil_id` 外键按键集分页返回当前引用该线圈的配方；语义 `CURRENT_RECIPE_COIL_REFERENCES`，`sort=id_desc`。用户问“这个线圈/定子被哪些配方用”时先用 `search_coils` 取得方案 ID，再用本工具反查；不得用 `get_all_recipes` 全量列表代替（真实库上该工具的原始正式回执是 120,990 bytes、模型视图是 22,591 bytes，但它只是配方目录的单向列表，不带关系完备性认证，无法区分 `coil_id` 为空而仅用 `coil_spec`/`coil_sheets` 旧字段声明该线圈的行；本工具为 644 bytes 且返回 `setCompleteness`）。工具 schema **只接受 `coilId`**：翻页由系统在一次调用内确定性完成（最多 8 页、每页 50 条、累计 < 32 KB），模型不控制也不应请求分页。读取结果先按 `BoundedRelationResultV1` 契约重新校验，未通过校验的响应一律拒绝采用。返回 `count/returnedCount/totalCount/hasMore/complete/setCompleteness/pagesFetched/serializedBytes/unconfirmedLegacyReferences/data[{recipeId,recipeName}]`；`complete=true` 仅在整段关系取回且全部引用可确认时为真；`setCompleteness` 取值 `COMPLETE/PARTIAL/AMBIGUOUS_LEGACY_REFERENCE/REFERENCE_INCOMPLETE`，配方仅用 `coil_spec`/`coil_sheets` 旧字段声明该线圈而 `coil_id` 为空时列入 `unconfirmedLegacyReferences`，不得当作“不使用该线圈”。线圈不存在返回 `RELATION_NOT_FOUND`，未提供合法 `coilId` 返回 `RELATION_REQUEST_INVALID`。该能力只读，不写库。
- `get_recipe_parts` / `get_recipes_by_part`：私有 Ontology 路由的有界配方—零件双向读取。两者分别只接受已由正式身份读取确认的 `recipeId` 或 `partId`，调用 `POST /api/relations/resolve` 的 `recipe.contains_part` / `part.contained_in_recipe`。执行器逐页校验 `OntologyRelationResultV1`，最多 8 页、累计小于 32 KB；任何缺失、歧义、非 canonical root、契约错误或不完整旧引用都不会被提升为完整结论。反向读取只忽略与当前 canonical 零件明确无关的旧行；可能指向当前目标但缺少 canonical ID 的引用仍 fail closed。工具不进入 MCP 目录，也不加入 Legacy/OFF 自动短名单。
- `get_recipe_detail`：按配方 ID、名称或可唯一匹配的简称定位后调用 `/api/recipes/:id` 读取正式 BOM 明细；名称匹配忽略大小写，并先剥离查询串末尾的口语语气助词与标点（`V550的`、`V550 吧`、`V550呢？`），原样名称仍优先精确匹配，正式名称本身以这些字结尾时不会被误剥离；剥离后零命中回答“不存在”并报出剥离后的查询（不把用户原话片段当实体名回显），多条命中返回结构化候选并等待下一轮选择。型号简称在正式配方目录查不到时，失败回执另附 `crossCatalogCandidates`（正式泵壳模板与零件目录中的同名候选，只含身份字段，不含价格与库存）与 `crossCatalogHint`，`error` 保持原“未找到”结论不变。工具响应只输出标准 camelCase，不向 MCP/AI 扩散正式 API adapter 为旧调用方保留的 `Id/CreatedAt/UpdatedAt`。`includeCurrentCost=true` 时调用 `/api/recipes/current-costs` 并按 `recipeId` 选择同一配方，返回 `currentCost.currentTotalCost/sourceOfTruth=costEngine/costBasis=currentFullCost/asOf`；~~`currentCost.unitCost`~~ 仅为 DSH、Claude Code 等现有 Agent 的一个兼容周期废弃别名，值必须与 `currentTotalCost` 相同，调用方应迁移到 `currentTotalCost`，确认外部 Agent 无依赖后删除。`/api/recipes/:id/cost-preview` 只用于带覆盖参数的报价/试算，保存成本和空覆盖试算均不能冒充当前完整成本。
...
- `preview_recipe_cost`：复用 V3 统一资源解析器按配方 ID、名称或可唯一匹配的简称解析已有配方。没有覆盖参数时调用 `/api/recipes/current-costs` 并返回 `currentTotalCost/sourceOfTruth=costEngine/costBasis=currentFullCost`；私人助理默认 `useRecipeBaseline=true`，有覆盖时调用 `/api/recipes/bom-draft`，按配方完整配置与当前价格重算，返回 `costBasis=configuredBomDraft`，包装按角色替换。API/MCP 未传该选项时保留兼容行为：存在覆盖参数时才调用 `/api/recipes/:id/cost-preview` 并返回同一主字段 `currentTotalCost` 及 `costBasis=overridePreview`。~~`unitCost`~~ 在两种口径中都只保留为与 `currentTotalCost` 等值的一个兼容周期废弃别名，调用方完成迁移后删除。名称匹配忽略大小写，剥离查询串末尾的口语语气助词与标点后仍可唯一绑定（正式名称本身以这些字结尾时原样优先），零命中报“不存在”、多候选返回结构化候选，用户以序号、完整名称、后缀或规格确认后绑定正式 ID 并继续原成本目标。型号简称在正式配方目录查不到时，失败回执附 `crossCatalogCandidates/crossCatalogHint`（正式泵壳模板与零件目录中的同名候选，只含身份字段），`error` 保持原“未找到”结论。
- `compare_recipe_scenarios`：只供 Task V2 的 Native 受限工具面调用，不加入冻结的 Legacy `AI_TOOLS` 或 MCP 目录。它调用 `/api/recipes/:id/scenario-compare-preview`，固定使用 `version=1` 与 `CURRENT_REBUILT`；输入不能提供成本、单价、铜价或未开放配置。正式 executor 回执保留 `readSetId/readSetHash` 与 `live_business` provenance。
- `preview_profitability`：只供 Task V2 的 Native 受限工具面调用。它调用 `/api/cost/profitability-preview`，基于同一次情景比较的完整正式成本回执生成 CNY 毛利试算；不接收客户端成本或旧 `margin`，不计算净利润。
- `preview_virtual_readiness`：只供 Task V2 的 Native 受限工具面调用，不加入冻结的 Legacy `AI_TOOLS`。它调用 `/api/inventory/virtual-readiness-preview`，必须携带正式配方/情景来源与数量来源；按当前库存扣除活动订单占用，只报告库存管理物料齐料，不创建订单、预留、采购或库存变更，也不声称产能或交期。
...
- `compare_recipes` / `explain_cost_change`：统一调用 `/api/cost/recipe-difference`，按当日完整成本比较两个配方，包含工资、表面处理和管理费；两者不再各自重算配件小计。配方选择器按“**canonical ID 优先、名称回退**”解析：全数字选择器走正式主键匹配（Native Task 在已经 canonical resolved 后只发送 `/recipes` 回执里的主键，`13` 与回执里的 `id: 13` 视为同一证据），非数字选择器按名称精确匹配、再退化为子串匹配；名称命中多条时整体失败（`RECIPE_SELECTOR_AMBIGUOUS`），不做模糊选择、不取第一条。两项能力都登记 `dataMode=live`、`resultProvenance=live_business` 和 `sourceOfTruth=costEngine`。差额统一按“对比配方（配方2/right）减基准配方（配方1/left）”计算，正数表示对比配方更贵。
...
普通 AI 对话（`aiAssistantRuntime`）的只读结果预算分两层，两层管的是不同的东西。第一层是**原始回执工程安全上限** `1048576 bytes`（1 MiB）：只拦正式 API 异常返回数十 MB 的情况，超限返回 `AI_QUERY_RESULT_TOO_LARGE` 且 `reason=RAW_RECEIPT_SAFETY_LIMIT_EXCEEDED`；通过的回执原样进入本轮 `toolResults`。第二层是**送给模型的上下文预算**（单条 96 KiB、本轮累计 96 KiB）：判定对象是 `modelResultView` 的投影（列表只保留身份与标量摘要，嵌套明细留在原始回执），单条超限 `reason=MODEL_VIEW_RESULT_TOO_LARGE`，累计超限 `reason=CUMULATIVE_MODEL_VIEW_BUDGET_EXCEEDED` 并关闭本轮后续读取。原始回执尺寸本身**不**决定结果是否有效，因此带投影的大列表（例如 `get_all_recipes` 全量配方目录）会正常投递给模型。所有拒绝都保留正式执行证据、不删除任何业务字段，要求改用正式筛选、显式 `limit` 或单资源详情；原始回执始终完整保留在本轮 `toolResults` 中，供金额守卫、canonical 身份核对和执行证据使用。该边界与 MCP 的独立结果上限职责分开；AI 调度器 V3（`aiAgentRuntimeV3`）另有一条独立的、按原始回执与合并证据计算的 `262144 bytes` 上限（`MAX_AI_READ_TOOL_RESULT_BYTES`）。`get_quotation_detail` 的报价 ID 必须由用户明确给出、来自当前报价页面，或来自本轮已验证的 `search_quotations` 正式结果；模型猜测 ID 会被阻止。
...
已有业务引用的名称读取：服务端从保存的 ID 或来源哈希/规格修订仍有效的正式绑定读取现名。BOM、订单、报价与采购 JSON 返回当前名称，改名时保留 snapshotName；配方/模板另返回 snapshotPartsJson（模板还含 snapshotShellComponentsJson），订单/报价返回 snapshotItemsJson，订单还返回 snapshotPurchaseListJson，均为原始保存 JSON；数据库原 JSON 和 order_revisions 不改，金额及库存换算不随名称变化。普通来源更新由 `safeUpdate` 在同一事务内续接仍有效且所在引用对象完全未变的绑定，并强审计旧绑定退役与新版本绑定；待办、状态、备注或其他采购行变化不会丢失未变引用。引用对象变更、数组位置替换、目标规格失效或来源软删除不续接；续接失败回滚来源更新。新选料仍要求 ID/现名/供应商一致，不能以旧名称更换实物。无 ID 且未核实的歧义旧引用继续拒绝，不能猜测供应商。
