...
> 更新于 2026-08-16。本文只描述当前生效的 HTTP 接口事实。强制规则见 [API 统一契约](./api-contract.md)，变更流程见 [API 变更 SOP](./api-sop.md)，业务口径见 [README.md](./README.md)，未完成风险和优化顺序见 [当前技术债与优化清单](./technical-debt.md)。
...
MCP 变更在发布到 Mac Mini 前运行 `npm run verify:mcp-local`：它组合协议/安全单测、官方 conformance 场景，以及基于临时数据库和隔离 API 的真实 HTTP 验收；Hermes 兼容的 2025 客户端与通用 2026 客户端分别调用全部 12 个工具。正式环境部署后只做发现、一个代表性只读调用、401 和身份日志的最小冒烟，不再用正式库重复完整回归。详细流程和 Windows conformance CLI 的退出兼容边界见 `docs/mcp-development-guide.md`。
