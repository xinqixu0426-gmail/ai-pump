...
AI 内部关系观察由独立环境开关 `AI_ONTOLOGY_RELATION_SHADOW_ENABLED=false` 控制，默认关闭；开启时仅在正式回答完成后异步比较一条已执行的 canonical 1-hop 关系。观察结果不进入模型上下文、工具响应、回答或证据，也不改变本节 HTTP/AI/MCP 契约。没有新增工具、客户端字段或可达的 `/api/ontology`、`/api/relations`；`relationReadRouter` 仍未挂载。只读 worker、比较契约、隐私和验收结果见 [Ontology Runtime Shadow V1](ontology-runtime-shadow-v1.md)。这不是新的业务查询或维护能力，不登记虚构 API。
