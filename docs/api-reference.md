...
| `POST` | `/api/ai/tasks/:taskId/write-preview` | `TaskWritePreviewRequestV1` | N6.1 仅为 `adjust_part_stock` 或 `batch_update_prices` 生成正式预检确认卡。任务必须为 owner-bound `DETACHED`、`CONFIRMATION_REQUIRED`，且目标为 `PREPARE_CHANGE`/`APPLY_CHANGE`。服务端将 `{ taskId, planRevision, goalKey }` 绑定到 token 的私有 executionContext，仅持久化 operationId 关联；卡片不执行写入，token、预检上下文、lease、参数和写权限不进入公开任务视图。过期、错主体、版本/参数漂移、旧卡或进程重启均须重新预检。AI Native rollout 还要求当前请求启动快照同时为 `AI_NATIVE_MODE=owner`、正式 Owner JWT 和 `AI_NATIVE_WRITE_ENABLED=true`；默认 `false`，请求体、模型内容或 header 不能改变该开关。 |
| `POST` | `/api/ai/tasks/:taskId/write-execute` | `TaskWriteExecuteRequestV1` | N6.2 只执行同一 owner-bound task 已预检的 `adjust_part_stock` 或 `batch_update_prices` 确认卡。服务端在正式命令发送前持久化 COMMAND step、确认 operation 关联与预检幂等键；实际写入仅经 `executeConfirmedAiTool` 与原正式业务 API。响应丢失不会生成新 operationId 或重放命令，而是进入 `RECONCILING`。响应只返回脱敏正式 operation/audit 摘要。除 N6.2 原有保护外，Native rollout 必须同时通过 `AI_NATIVE_MODE=owner`、正式 Owner JWT 和独立的 `AI_NATIVE_WRITE_ENABLED=true`；`owner` 模式本身绝不启用写入。 |
| `POST` | `/api/ai/tasks/:taskId/write-reconcile` | 无 | 仅对 owner-bound `RECONCILING` task 的唯一 COMMAND step 进行当前主体受限的只读正式 operation 查询。查到已完成的可信 operation/audit 回执才完成目标；未查到、处理中或歧义时保持 `RECONCILING`，不得换 key 重试或推断成功。 |
