...
> 更新于 2026-08-29。本文只描述当前生效的 HTTP 接口事实。强制规则见 [API 统一契约](./api-contract.md)，变更流程见 [API 变更 SOP](./api-sop.md)，业务口径见 [README.md](./README.md)，未完成风险和优化顺序见 [当前技术债与优化清单](./technical-debt.md)。
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `GET` | `/api/coils` | 查询参数 `spec?/sheets?/material?/slotType?` | 能力 `coils.list`。`coilQueries` 按类型化字段精确筛选并只读返回完整线圈方案档案，字段含 `diameterMm/commonName/material/slotType/schemeName/schemeStatus/stock`、`pricingMode=calculated\|kit`、`kitPrice`、成本组成、`defaultWireGauge/defaultCapacitor` 以及 `mainWireGauge/mainWireData/auxWireGauge/auxWireData`；`stock` 单位为套，`sheets` 必须为正整数。Query 严格只读，事实来源为 `coils + stator_variants`（标准直径、俗称和槽眼取自定子组合），风险 low，天然幂等、无并发/事务/审计要求，默认超时 15 秒；Web、AI 和内部调用共用 |
| `GET` | `/api/coils/variants` | 无 | `coilQueries` 只读返回定子组合列表；组合键为标准直径、材质和槽眼 |
| `POST` | `/api/coils` | `spec, diameterMm, material, slotType, sheets, schemeName?, schemeStatus?, pricingMode?=calculated\|kit, kitPrice?, unitPrice?, wireWeight?, copperBase?, coilFee?, rotorFee?, defaultWireGauge?, defaultCapacitor?, mainWireGauge?, mainWireData?, auxWireGauge?, auxWireData?, idempotencyKey?`；推荐请求头 `Idempotency-Key` | 能力 `coils.create`。`calculated` 要求 `unitPrice` 并按成本组成计算；`kit` 要求 `kitPrice>0`，`wireWeight/copperBase` 可选且必须为非负参考值，`cost` 严格等于套件价，`unitPrice/coilFee/rotorFee` 规范化为 0。材质仅支持钢带/冷轧，槽眼仅支持小眼/国标眼；正式方案会在同一事务内把同组合同片数的原正式方案改为测试方案。原线圈顶层字段继续返回，并增加标准命令回执 |
| `PATCH` | `/api/coils/:id` | 线圈 camelCase 字段（含 `pricingMode/kitPrice/wireWeight/copperBase`），`expectedUpdatedAt?`, `idempotencyKey?`；推荐请求头 `Idempotency-Key` | 能力 `coils.update`。修改定子组合、绕组方案或计价方式；相关成本字段变化时由服务端按当前模式重算。套件模式保留可选线重和铜价基数参考，但二者不进入成本，单片价和加工费清零。库存大于 0 或已有库存流水后，规格俗称、定子直径、片数、材质和槽眼被冻结，修改这些身份字段返回 `409`；应新建方案。新调用必须传资源版本；旧请求兼容执行并返回缺少并发保护 warning |
...
| `POST` | `/api/coils/calculate` | `{ spec, sheets, material?, slotType?, wireWeight?, copperPrice? }` | `sheets` 必须为正整数，线重和铜价必须为非负数字；只使用正式方案。精确命中 `kit` 时返回 `pricingMode/kitPrice/wireWeight/copperBase/totalCost`，其中线重和铜价基数是保存的可选参考值，忽略请求中的成本覆盖参数且 `totalCost` 始终等于 `kitPrice`；未精确命中时只允许 `calculated` 方案在同标准直径、材质和槽眼内插值或外推 |
...
| `POST` | `/api/model-variants` | `modelName, templateId` 必填；可带线圈、机筒、长螺丝、叶轮字段、`customFieldsJson` 和 `idempotencyKey?`；推荐请求头 `Idempotency-Key` | 能力 `model_variants.create`。新增常用配置预设；若模板含长螺丝且预设有机筒长度，会按参数化螺丝公式自动补齐对应长度的螺丝零件。常用配置预设、自动生成零件、operation 和全部强审计同一事务提交；历史顶层字段及响应顶层 `createdLongScrewParts` 保持兼容 |
| `PATCH` | `/api/model-variants/:id` | 同新增字段，另带 `expectedUpdatedAt?`, `idempotencyKey?`；推荐请求头 `Idempotency-Key` | 能力 `model_variants.update`。更新常用配置预设并可能沉淀新的长螺丝规格；新调用绑定资源版本，旧无版本请求兼容执行并返回 warning；历史兼容字段与 `createdLongScrewParts` 保持兼容 |
...
| `POST` | `/api/recipes/bom-draft` | `{ templateId?, modelVariantId?, customBarrelLength?, coilSpec?, coilSheets?, coilMaterial?, coilWireWeight?, hasFloat?, floatWire?, floatAccessoryType?, hasCable?, cableLength?, cableWire?, cableAccessoryType?, packingParts?, optionalParts? }` | 经 `recipeQueries` 聚合正式模板、常用配置预设（历史类型 `model_variant`）、泵壳元数据、零件、系统设置和线圈，再委托 `recipeBomEngine` 生成标准化 BOM；不写库。启用电缆时，`cableLength` 必须为正数、`cableWire` 必须形成有效电缆型号、`cableAccessoryType` 只能是 `standard/xinjie`，错误返回稳定 `CABLE_*` code。成品电缆统一由 `costEngine` 按“每米单价 × 长度 + 全局铜套费”生成；名称和铜套费优先读取 `system_settings.cable_accessories`，型号 notes 和旧“电缆配件费”仅作兼容回退。电缆项返回 `cableLength/cableUnitPrice/cableAccessoryFee/inventoryUnit=m/cablePriceSource/cableAccessorySource/formulaVersion/pricingComplete`；目录缺少每米价时草稿保留 `pricingComplete=false` 供诊断，正式保存仍由未定价检查阻断。新界式浮球附加费只读取全局 `float_accessory_delta`，调用方不得自行提供第二套差价。`coilWireWeight` 为客户指定线重，会重算线圈成本。不锈钢机筒泵壳使用套件整体价时，`customBarrelLength` 会按 150mm 基准、每增加 10mm 加 1 元修正泵壳套件快照价，加价直接反映在“泵壳套件”这一行的 `snapshotPrice` 和 `shellPrice` 上。自由组合模板中只有 `componentType=stainlessStretchBarrel` 的“不锈钢拉伸筒”组件会用 `customBarrelLength/modelVariant.barrelLength` 换算 cm 数量，并触发长螺丝长度联动；铝机筒、铁机筒按普通固定组件处理。历史 `isStainlessStretchBarrel=true` 数据继续兼容。自由组合组件取价只读取“泵壳搭配”分类。`coilSnapshot` 返回 `wireGauge/defaultCapacitor/pricingMode/kitPrice/totalCost/coilId/source/formula`：套件模式只在正式方案精确命中时返回 `pricingMode=kit` 和套件价，`coilId` 是后续库存身份；对应 `parts[]` 线圈行同步返回 `coilId/inventoryType=coil/pricingMode/kitPrice`，未命中正式库存身份时 `inventoryType=none`。返回的全部 `parts[]` 必须包含当前成本价 `snapshotPrice`，计算项或手动价需带 `formula/costSource/source` |
...
| `POST` | `/api/cost/full-estimate` | `{ recipeId? | recipeName?, stator?, statorMaterial?/material?, cableLength?, hasFloat?, floatWire?, cableWire?, floatAccessoryType?, cableAccessoryType?, boxType? }` | `costQueries` 为 AI/N8N 编排配方配件、线圈和动态配置；`recipeId/recipeName` 至少提供一个，名称只允许唯一匹配正式配方。未找到、匹配多条或把泵壳模板名当配方名时整体返回 4xx，禁止保留 `recipeCost.error` 后把缺失配方按 0 元静默合计。兼容字段 ~~`pumphousing_model`~~ 仅在 route adapter 中按 `recipeName` 读取一个周期，不代表泵壳模板；泵壳本体使用 `POST /api/recipes/bom-draft`/`preview_pump_shell_cost`。成功响应标记 `sourceOfTruth=costEngine/costBasis=composedEstimate`；`statorCost` 返回 `pricingMode/kitPrice/wireWeight/copperBase/coilId/inventoryType`，精确套件方案保留正式按套库存身份且参考字段不参与成本；各分项公式仍来自正式成本 services |
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `POST` | `/api/rotor/draw-preview` | 结构化出图参数；可带 `drawingName/drawing_name`、`drawingText/drawing_text`；轴承支持 `6201`–`6205`、`6303`、`6304` 及对应三位别名/密封后缀 | 能力 `drawings.rotor.generate_pdf` 的只读预览。服务端规范化轴承和尺寸，返回 `confirmationToken/operationId/inputHash/expiresAt/suggestedIdempotencyKey/params`，不写库、不启动 FreeCAD |
...
MCP 写目录、确认协议、executor 或正式 command 变更还必须运行 `npm run verify:mcp-write-local`。该门禁以 `scripts/mcp-write-acceptance-manifest.cjs` 为显式验收清单，并与上述 19 个正式写工具做严格集合比对；清单还把本轮之前已完成人工灰度验收的 9 项作为测试基线，将批次候选 10 项按“订单与报价转单（7 项）/文件归档（1 项）/转子出图与打印（2 项）”形成三个场景，二者必须无重复、无遗漏。该本地分组不是生产 allowlist 的事实来源，实际生产权限始终由部署后的认证身份目录核对。快速协议矩阵逐工具覆盖 scope 拒绝、逐工具 allowlist 拒绝、Preview、HMAC 主体/工具/参数绑定、明确接受、确认层重放和拒绝无副作用。随后 `scripts/run-mcp-write-local-e2e.cjs` 创建全新临时 SQLite、随机 localhost 端口和 2025/2026 两代真实客户端；2026 写客户端只取一次权威目录快照，让 19 个工具逐一完成 MCP form elicitation → 正式 executor/API → operation/audit → Query/数据库回读闭环，并让剩余 10 项逐一完成真实 form decline，逐项比较数据库与外部命令计数为零。2025 客户端仍真实完成只读调用并拒绝隐藏写工具。五个代表性正式命令使用同一 `requestState/inputResponses` 做真实幂等重放，订单、库存、配方、文件、打印覆盖真实失败且零副作用；`update_recipe` 额外从空规格基线改为临时非空规格，再以 `clearSpec:true` 完成第二次原生确认和正式恢复，核对包含包装箱型与旧喷漆工资迁移状态的完整业务快照恢复、零件目录零副作用及第二次持久 operation。`delete_recipe` 使用同一临时配方完成正式删除 Preview、原生确认、软删除 Command、operation/audit、详情 404、列表数量精确减一和零件目录零副作用闭环；`delete_part` 使用同轮临时零件完成唯一目标解析、正式删除 Preview、版本与哈希绑定、原生确认、软删除 Command、operation/audit、目录不可见和其他零件零副作用闭环。报价转订单和打印分别核对来源业务字段及完成任务的 `jobId/PDF`。订单与报价转换的不可逆测试状态随临时数据库整体销毁；FreeCAD、macOS `lp` 及 Windows Sumatra/Edge/rundll32 均由可计数替身拦截，未知外部命令 fail-closed，并回读异步终态。报告分别写入 `logs/mcp-write-local-latest.json` 和 `logs/mcp-write-local-e2e-latest.json`，包含目录快照、候选清单、三场景结果和逐工具拒绝证据，并明确记录 `productionTouched=false/physicalSideEffects=false`。本地 19/19 通过只证明代码链路具备受控写入条件，不构成生产写开关、身份或逐工具 allowlist 的授权；生产 `print_rotor_drawing` 默认只验收拒绝路径，报价转单和归档也只能使用专用灰度数据。
...
| `coils.create` | HTTP/Web | command/write | `stator_variants` + `coils.pricing_mode/kit_price/cost` | medium | 页面保存是明确动作 | 无 | 90 天持久化幂等；新资源无版本 | 定子组合、正式方案替换、模式化成本、operation 和逐项强审计同一 SQLite 事务 | 默认 HTTP |
| `coils.update` | HTTP/Web | command/write | 当前线圈 + 定子组合 + 计价方式 + 库存流水 | high | 页面保存是明确动作 | 无 | 90 天持久化幂等 + `expectedUpdatedAt` | 身份冻结检查、模式化成本重算、正式方案替换、operation 和强审计同一 SQLite 事务 | 默认 HTTP |
...
- `search_coils`：调用 `/api/coils` 读取线圈/定子方案的实时完整档案，按 `spec/sheets/material/slotType` 类型化筛选；除库存、成本和方案状态外，还返回默认电容、默认搭配电缆线径及主副线漆包线线径/绕组数据。测试或停用方案继续携带明确 `schemeStatus`，空字段表示该方案未填写，不能推断系统没有该字段。不用知识快照回答当前线圈档案、库存或成本。
