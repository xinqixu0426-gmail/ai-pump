...
当前源码共有 245 个 Express 路由声明。表内出现不代表推荐新调用：标为兼容或观察的入口仅供现有调用方迁移，新增页面、AI 工具和内部服务必须使用标准入口。
...
库存引用规则：普通件复用 `bomPartIdentity`，明确 partId 不回退；名称-only 必须唯一命中，并尊重明确供应商。线圈按 BOM coilId、配方 coilId、最后无 ID 的精确规格唯一候选/唯一默认顺序解析，明确失效 ID 不回退。`referenceStatus` 为 resolved/resolved_legacy/missing/ambiguous/inactive/invalid_id/identity_mismatch/invalid_reference/invalid_stock/not_tracked；`status` 为 in_stock/out_of_stock/missing/needs_review/not_tracked，`inventoryType` 为 part/coil/none。身份异常返回 currentStock:null，已有 ID 的名称冲突可附目录现名，但不证明改名授权或可继续入库。加工费、未绑定外包装估算及显式非库存行不显示缺货。损坏 BOM 返回422，超过10000行返回413；普通读取不修复快照。
