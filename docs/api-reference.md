...
| `orders.list` | HTTP/Web/`get_recent_orders` | query/read | `orders` + 实时采购平衡 | low | 否 | 不适用 | 不适用 | 严格只读 | 默认 HTTP |
