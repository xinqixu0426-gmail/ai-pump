...
活动订单的采购名称视图由 `orderPurchasePlanning` 调用 `buildSavedBalancedOrderPlanViews`，在同一只读事务中读取订单、目录和库存。保存的 partId/coilId 用于读取当前名称，采购行 `model` 返回现名、`snapshotName` 保留保存 BOM 中的名称；角色名称不随物料改名替换。采购总览复用该视图，配置身份、原采购行 ID、实际供应商、实际价格和已下单/到货/入库进度保持。浮球与电缆配置不依赖新显示名重新推断。失效 ID、供应商冲突或旧进度无法唯一接续时明确失败，不自动绑定或合并。已关闭/已取消订单尚不应用此现名视图；生产准备及写操作仍走严格计划构建，不因 Query 使用现名而放宽校验。
