...
> 更新于 2026-08-17。本文只描述当前生效的 HTTP 接口事实。强制规则见 [API 统一契约](./api-contract.md)，变更流程见 [API 变更 SOP](./api-sop.md)，业务口径见 [README.md](./README.md)，未完成风险和优化顺序见 [当前技术债与优化清单](./technical-debt.md)。
...
当前源码共有 218 个 Express 路由声明。表内出现不代表推荐新调用：标为兼容或观察的入口仅供现有调用方迁移，新增页面、AI 工具和内部服务必须使用标准入口。
...
| `PATCH` | `/api/parts/:id` | 请求头 `Idempotency-Key`；可更新字段及 `expectedUpdatedAt` | 能力 `parts.update`。资源版本、零件更新、operation 和强审计同一事务；原路径及顶层零件字段保持兼容。泵壳分类零件修改型号时，若旧型号已无其他有效泵壳零件，则在同一事务同步引用该旧型号的泵壳模板并返回 `linkedTemplateIds`，避免不锈钢机筒属性和成本规则失联。历史请求仍可提交 `stock`，但响应带 `part_stock_patch_compatibility`；Web、AI 和新增调用必须改用 `/batch-stock-preview` → `/batch-stock` |
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `GET` | `/api/coils` | 查询参数 `spec?/sheets?/material?/slotType?` | 能力 `coils.list`。`coilQueries` 按类型化字段精确筛选并只读返回完整线圈方案档案，字段含 `diameterMm/commonName/material/slotType/schemeName/schemeStatus/stock`、成本组成、`defaultWireGauge/defaultCapacitor` 以及 `mainWireGauge/mainWireData/auxWireGauge/auxWireData`；`stock` 单位为套，`sheets` 必须为正整数。Query 严格只读，事实来源为 `coils + stator_variants`（标准直径、俗称和槽眼取自定子组合），风险 low，天然幂等、无并发/事务/审计要求，默认超时 15 秒；Web、AI 和内部调用共用 |
...
| 方法 | 路径 | 入参 | 返回/说明 |
|---|---|---|---|
| `GET` | `/api/recipes` | 可选 query：`keyword`、`hasTechnicalFiles=true/false` | 经纯读 `recipeQueries` 返回配方列表；默认列表和筛选结果均返回 `technicalFileCount`，`keyword` 对配方名称和规格做模糊筛选，`hasTechnicalFiles` 按有效技术档案关联筛选。标准字段含 `id/createdAt/updatedAt/customBarrelLength/longScrewExtraLength` |
...
| `POST` | `/api/recipes/:id/cost-preview` | `{ overrides: { coilSpec?, coilSheets?, coilMaterial?, coilSlotType?, hasFloat?, floatWire?, floatAccessoryType?, hasCable?, cableLength?, cableWire?, cableAccessoryType?, customBarrelLength?, packingPartsJson?, boxType?, surfaceTreatmentMode?, surfaceTreatmentCost? } }` | 报价和直接建单共用的客户配置试算。经 `costQueries` 读取正式配方，再委托 `dynamicCostPreview` 以保存成本为基线替换被覆盖的动态项；返回 `unitCost/parts/costSnapshot/warnings`，其中 `parts` 是应用覆盖后的可采购 BOM 快照。电缆覆盖与 BOM 草稿复用同一 `costEngine` 计价和 `CABLE_*` 校验，快照保留价格、配件来源及公式版本。包材按完整有效清单重算，表面处理替换原工艺成本；变更后的线圈无法计价时返回 `422 COIL_CONFIGURATION_UNPRICED`，插值或外推但未绑定正式库存方案时返回 `coil_inventory_scheme_required` warning |
...
| `POST` | `/api/quotations/save-payload-draft` | `{ customerId, status?, items|itemsJson, remark?, attachmentFileIds?, attachmentSummary?, attachmentSourceFileIds? }`；明细可含 `overrides`，`qty` 可省略或为 `null`，附件最多 20 个，摘要依据最多 4 个且必须属于本次附件 | `quotations.create/update` 的正式只读预览。后端按每个有效 `baseRecipeId` 通过共享客户配置快照服务重新调用权威成本逻辑，并校验询价附件和摘要来源；报价明细保存 `overrides/configurationSnapshot/warnings`、单位成本、出厂单价、`bomSnapshot/costSnapshot`。转单后才映射为订单字段 `configurationOverrides/configurationSnapshot/configurationWarnings`。客户包材覆盖只接受型号、供应商、数量和展示语义，忽略请求中的 `snapshotPrice/costSource` 并按正式零件目录重新定价。只有全部明细数量均为正数时才返回数值型 `totalCost/totalPrice`；否则两项为 `null`、`quantitiesConfirmed=false` 并返回 `quotation_quantity_pending` warning，避免把缺失数量默认成 1。返回规范化附件字段、绑定全部保存语义的 `previewHash` 和 `suggestedIdempotencyKey`；不信任前端成本且不写库 |
...
| `POST` | `/api/quotations/:id/order-draft` | `{ itemQuantities?: [{ quotationItemId, qty }] }`，最多 100 项，数量必须为正整数 | 从报价单价及已展开的配置、BOM 和成本快照生成订单预览、采购清单和待办，原样保留 `configurationOverrides/configurationSnapshot/configurationWarnings` 供订单追溯。报价明细没有保存数量时必须通过 `itemQuantities` 补齐；缺失返回 `422 quotation_item_quantity_required`，重复、未知明细或无效数量返回 400。响应回传规范化 `itemQuantities`、`capabilityId`、报价 `expectedUpdatedAt`、确认内容 `previewHash` 和一次性建议 `suggestedIdempotencyKey`。`previewHash` 绑定最终数量及正式订单/BOM/采购/待办语义；旧报价缺少快照时临时回退配方 BOM 并标记 `legacy_recipe_fallback`；不写库 |
...
| `POST` | `/api/orders/save-payload-draft` | `{ customerId?, customerName?, contractNo?, remark?, status?, items: [{ recipeId, qty, profitMargin?, unitPrice?, configurationOverrides? }], purchaseList?, todos? }` | 能力 `orders.create` 的正式只读预览兼保存 payload 草稿。新增调用提交稳定 `customerId`；历史 `customerName` 仅用于精确兼容解析。每个产品必须引用有效配方；没有配置覆盖时从配方保存快照重建名称、规格、完整 BOM 和单位成本，有覆盖时通过与报价共用的 `configuredRecipeSnapshot + costEngine` 重新生成并锁定 `configurationOverrides/configurationSnapshot/costSnapshot/partsJson/unitCost/configurationWarnings`。未知配置字段、未定价项目或无法计价的线圈会拒绝草稿；插值/外推线圈可保存但返回正式库存方案 warning，并在生产准备检查中继续阻塞。客户端 `unitCost/partsJson/recipeName/spec` 不作为事实。采购清单和待办始终按服务端最终 BOM、供应商身份及全部活动订单实时库存重建，客户端同名字段被忽略并返回 warning。返回 `preview=true`、`changes/warnings`、`previewHash` 和建议幂等键，不写订单、operation 或审计；直接建单固定从“待确认”开始 |
...
MCP 变更在发布到 Mac Mini 前运行 `npm run verify:mcp-local`：它组合协议/安全单测、官方 conformance 场景，以及基于临时数据库和隔离 API 的真实 HTTP 验收；Hermes 兼容的 2025 客户端与通用 2026 客户端分别发现并调用全部 45 个只读工具，2026 写身份再完成一次 `sync_factory_knowledge` 正式 Preview → form elicitation → Command → operation/audit 回执闭环。隔离进程仅为连续调用把测试限流提高到 600，所有写入只发生在临时数据库副本，生产默认值和源数据库不变。正式环境部署后的日常发布自动运行 `npm run verify:mcp-prod-read`，严格核对 45 个只读工具及 annotations；若验收身份处于写灰度，只允许额外出现其逐工具 allowlist 中的写工具，未知或越权目录会失败。随后在一个连接内复用三个成本场景，再以 18 次调用覆盖 17 个代表工具及库存、配方、客户/报价、订单/采购、管理/质量、知识和出图历史；每个结果必须带已验证执行证据、能力 ID 和正式数据源，并核对 `get_recipe_detail` 没有大小写重复键、其 `currentTotalCost` 及无覆盖的 `preview_recipe_cost` 均与 `compare_recipes` 的 `currentFullCost` 一致，覆盖试算也必须以 `currentTotalCost` 为主字段并保留等值废弃别名。`compare_recipes` 与 `explain_cost_change` 必须同时返回 `dataMode=live`。最坏 35 个请求低于生产每分钟 60 次限制，综合脱敏报告写入 `logs/mcp-production-read-latest.json`，成本子报告仍写入 `logs/mcp-production-cost-latest.json`。生产空数据不视为协议失败，也不为覆盖详情造数据；全部 45 个工具和缺价路径仍在隔离库验收。生产写开关的发现和单笔可回滚验收仍必须另行明确批准。详细流程和 Windows conformance CLI 的退出兼容边界见 `docs/mcp-development-guide.md`。
