...
模板、配方和常用配置新建必须提供 `naming:{ruleId,spec}`，规则分别为template/recipe/model-variant；不带规格的自由名称新建返回400 CATALOG_NAMING_REQUIRED。服务端生成名称并在创建事务保存结构化身份档案，普通PATCH不能绕过改名。配方命名只手填series/configuration，定子、片数及已知机筒长度取保存参数；重复提供这些值且不一致返回422 CATALOG_NAMING_SPEC_MISMATCH。新建externalModel可独立提供，缺省保留原name或生成名，草稿hash绑定命名信息与对外型号。整套模板必须提供具体shellPartId，类别/停用校验后保存独立绑定，多供应商不能任取。存量普通编辑保留原入口。AI create_recipe同步要求naming、coilSpec、coilSheets及正式线圈ID或方案系列，通过正式保存草稿/API建档。
