...
| `POST` | `/api/orders/save-payload-draft` | `{ customerId?, customerName?, contractNo?, remark?, status?, items: [{ recipeId, qty, profitMargin?, unitPrice?, configurationOverrides? }], purchaseList?, todos? }` | 能力 `orders.create` 的正式只读预览兼保存 payload 草稿。新增调用提交稳定 `customerId`。每个产品必须引用有效配方；没有覆盖时使用配方保存快照，有覆盖时通过与报价共用的 `configuredRecipeSnapshot + costEngine` 先校验配方配置范围，再重新生成并锁定 `configurationOverrides/configurationSnapshot/costSnapshot/partsJson/unitCost/configurationWarnings`。未知字段、规则外配置、目录零件身份歧义、未定价项目或无法计价的线圈会拒绝草稿；线圈变化同步替换依赖电容。客户端成本、BOM、采购清单和待办不作为事实，服务端按最终稳定身份和实时库存重建。返回 `preview=true`、`changes/warnings`、`previewHash` 和建议幂等键，不写订单、operation 或审计；直接建单固定从“待确认”开始 |
