...
AI 内部关系观察由独立环境开关 `AI_ONTOLOGY_RELATION_SHADOW_ENABLED=false` 控制，默认关闭；开启时仅在正式回答完成后异步比较一条已执行的 canonical 1-hop 关系。观察结果不进入模型上下文、工具响应、回答或证据，也不改变本节 HTTP/AI/MCP 契约。没有新增工具、客户端字段或可达的 `/api/ontology`、`/api/relations`；`relationReadRouter` 仍未挂载。独立子开关 `AI_ONTOLOGY_RELATION_BINDING_SHADOW_ENABLED=false` 同样默认关闭；父开关开启时，子开关只增加确定性的关系意图与 canonical root 绑定观察，不增加模型调用或改变正式执行链。只读 worker、比较契约、隐私和验收结果见 [Ontology Runtime Shadow V1](ontology-runtime-shadow-v1.md)，绑定契约、拒绝规则和冻结语料比较见 [Ontology Relation Binding V1](ontology-relation-binding-v1.md)。另有默认关闭的 `AI_ONTOLOGY_2HOP_SHADOW_ENABLED=false`，必须同时开启前述两个开关才观察一条白名单两跳路径；仅复用只读 Resolver，不增加工具/模型调用或影响一跳比较。路径策略、fan-out 上限、部分结果及独立 oracle 见 [Bounded 2-Hop Traversal Shadow V1](ontology-traversal-shadow-v1.md)。这些不是新的业务查询或维护能力，不登记虚构 API。 独立私有环境开关 `AI_ONTOLOGY_RELATION_ROUTING_CANARY_ENABLED=false` 默认关闭，按项目统一规则仅 `true` 启用（与三个 shadow 开关共用 `api/services/environment.cjs` 的 `isEnvFlagEnabled`），不得通过代码默认开启。ONT-P7 后该开关对 `recipe↔coil` 家族已成为**可受控开启的正式关系路由**：provider 资格为 `local`/`local-first`/`deepseek`（未验证 provider 如 kimi 不在内），其中 `local`/`local-first` 仍要求本地 shortlist 启用，`deepseek` 不要求，且 `auto` 会解析为实际承接请求的 provider；命中时由 canary execution profile 决定关系意图、所需正式只读查询与完成度，Legacy 检测器/修复不再参与该请求决策（不双跑）。未命中、缺少或歧义 canonical root、不支持的关系、绑定前内部故障以及开关关闭时一律回到原 Legacy 路径。P4 绑定成功后，canary 会先由软件按 profile 的 `requiredReads` 确定性排队执行所需正式只读查询（仍经过既有工具白名单、schema、标识接地、只读执行器与执行证据校验），再由模型合成最终回答，因此不引入额外模型规划/重试轮次。没有新增客户端字段、能力、HTTP/AI/MCP 工具或证据契约。语义边界、Legacy Oracle、确定性 A/B、严格 local 门禁（本机不在模型主机网段，DEFERRED，不接受云端 fallback 顶替）与 HTTP/SSE 真实运行门禁见 [First Relation Routing Migration V1](ontology-first-routing-migration-v1.md)。
