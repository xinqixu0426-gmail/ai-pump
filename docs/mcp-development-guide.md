...
兼容的 2025 客户端和通用 2026 客户端分别发现并调用全部 49 个只读工具。
...
`compare_recipes` 和 `explain_cost_change` 都必须是 `dataMode=live`。目录验收固定要求 49 个只读工具完整且
...
制造生产数据；全部 49 个只读工具和缺价失败路径继续由隔离套件覆盖。
